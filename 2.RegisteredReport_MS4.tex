...
repeated stimuli, but ignore the ordinal and geometric properties of
spatial-forms. This registered report consists of two phases In
\emph{phase I}, we optimise SSS diagnostics from consistency test by
extracting novel tests at the spatial-form level. In this exploratory
phase we evaluated classification performance of old and new methods in
a large (N = 685) aggregated sample from four available and independent
datasets. We then harness a geographic toolbox to extract metrics from
the spatial-forms level. Finally, receiver operating characteristic
analyses are conducted to assess the discriminative performances of each
method. The results show that permuted topological validity of
spatial-forms and the perimeter between stimuli performs equally well in
detecting SSS. In confirmatory \emph{phase II}, we will examine
predictive validity of the selected methods on an independent dataset
that has yet to be collected. Findings from \emph{phase I} highlight the
relevance of topological principles for sequence-space representations
and aim to optimise the ability of consistency tests to detect SSS. }
...
spatial-forms can be complex and can have variable geometric forms.
These geometric forms are idiosyncratic and might be different for each
category (i.e. number-forms weekdays and months), with forms such as
ellipses, zig-zags or curbed lines
(\citeproc{ref-flournoy1893}{Flournoy, 1893};
...
\citeproc{ref-seron1992}{Seron et al., 1992}). Existing methods for
classifying SSS and controls based on consistency tests have some
limitations (\citeproc{ref-baron-cohen1987}{Baron-Cohen et al., 1987};
\citeproc{ref-brang2010}{Brang et al., 2010};
...
\& Simner, 2022}). Therefore, in an exploratory \emph{phase I} study, we
optimized the consistency methods from aggregated and already available
datasets. Then, in a confirmatory \emph{phase II} registered report
study, we aim to test and compare the optimal methods on data that has
yet to be collected.
\pdfcomment[icon=Check,color={0 0 1}]{NR1: However, it is not clear that Study 1 and Study 2 will both be in the same paper, while Study 1 is exploratory and Study 2 based on registration of the results from the first study confirmatory. I think, you must make clear that Study 2 is why we submit this manuscript as registered report.}

Spatial-forms are idiosyncratic,
\pdfcomment[icon=Check,color={0 1 0}]{CS2: I would mention this where you first say that SSS is idionsyncratic (the first sentence of this paragraph).}
...
\textcolor{teal}{Despite this heterogeneity, SSS responses are consistent, meaning that the spatial position to the same inducers should be constant in time.}

\subsection{Consistency test}\label{sec-consistency-tests}

\pdfcomment[icon=Note,color={1 0 0}]{RL1: I changed the order in the paragraphs so that I start with how synaesthesia is diagnosed in general (self-reports \& consistency). Then define consistency and how it is applied to GCS. Finally how it is tested in SSS.}

\pdfcomment[icon=Check,color={0 1 0}]{CS3: Overall, this section is great, but I wonder if it would be better to first describe consistency tests more generally and only later say how it is applied for SSS. I say this because at the moment, you talk about it in relation to SSS first, then GCS, and then back to SSS. Also, I would refer to grapheme-color synesthesia, not color-grapheme synesthesia. I think grapheme-color is the correct way to say it.}

\pdfcomment[icon=Check,color={0 0 1}]{NR2: All the information is there, but the strucutre is not yet optimal. The second sentence in the following paragraph can only be understood when one knows how a consistency tests works. -> Recommendation: I would start with a more general statement that synaesthesia is diagnosed on the basis of phenomenological reports, for instance by means of questionnaires, and consistency tests. Then, explain how consistency tests work. Then wirte about consistency tests in SSS.}

Individuals with synaesthesia are usually identified by the means of
...
et al., 2013}, \citeproc{ref-rothen2016}{2016}). An individual is
identified as a synaesthete based on the variability between repeated
responses to the same inducer or stimuli: less variable responses being
an indicator of the consistent responses characteristic of synaesthesia.
\textcolor{teal}{This is because synaesthetes can respond from their perceptual experience while non-synesthetes can only be consistent by relying on a memorization strategy}
\pdfcomment[icon=Check,color={0 0 1}]{NR3: Add explanation -> because synaesthetes can rely on experiences, but non-synaesthetes must rely on memory}.
Consistency tests have been proven effective for grapheme-colour
...
applied to characterize SSS from consistency tasks. However, consistency
tests for colour-grapheme synaesthesia generally show better performance
than the correspondent tests used for SSS (see Table~\ref{tbl-01}).

\begin{table}

{\caption{{Consistency tasks performances for grapheme-colour and
sequence space synaesthesia}{\label{tbl-01}}}
\vspace{-20pt}}
...
\bottomrule\noalign{}
\endlastfoot
& Grapheme-Colour Synaesthesia & Sequence-space Synaesthesia \\
...
\vspace{-20pt}
\noindent \emph{Note.} AUC~= Area Under the Curve
...
\emph{inducer}-\emph{concurrent} associations). Brang et al.
...
conservative, as it identified synaesthesia in 4 of 81 self-reported
...
consistency. Less consistent responses lead to smaller triangle
perimeters and area (\citeproc{ref-rothen2016}{Rothen et al., 2016}). A
cut-off has been proposed whereby individuals are classified as SSS if
their average response area is smaller than 0.203\% of the total screen
area (see also \citeproc{ref-ward2018}{Ward et al., 2018}).

\subsection{Limitations of current methods of consistency
tests}\label{limitations-of-current-methods-of-consistency-tests}

\pdfcomment[icon=Note,color={1 0 0}]{RL2: this whole section has been restructured and/or rewritten}

\pdfcomment[icon=Check,color={0 1 0}]{CS4: Would it be useful to compare SSS consistency tests to the GCS one as far limitaitons go? I am not sure though}

A first limitation of current consistency tests is that participants
that respond the same spatial position throughout the task in SSS, or
the same colour in grapheme-colour synaesthesia, will have artificially
high consistencies(\citeproc{ref-rothen2016}{Rothen et al., 2016}). This
problem has particularly been identified for SSS. Indeed, exclusion via
visual inspection of these responses for SSS lead to improved
classification performances using the area test (i.e.~Area Under the
Curve = 85\% Sensitivity = 87\%; Specificity = 81\%, , see Rothen et al.
(\citeproc{ref-rothen2016}{2016})).
\pdfcomment[icon=Check,color={0 0 1}]{NR4: Do these limitations apply in general or only to SSS consistency tests?}

A second limitation that is specific to SSS consistency tests, is that
certain spatial-forms might be favoured depending on the method
used(\citeproc{ref-ward2018}{Ward et al., 2018}). In particular for
spatial-forms with linear or highly regular spatial layouts,
...
and SSS (\citeproc{ref-ward2022a}{Ward, 2022}).

Third, current consistency tests evaluate the variability of responses
to the same stimuli, therefore the ordinality between stimuli is not
taken into account. Synaesthetic spatial-forms depend on ordinal rules
(e.g.~Monday \texttt{→} Tuesday \texttt{→} Wednesday). In the domain of
numerical cognition, ordinality has been identified as an important
information when processing sequences (\citeproc{ref-lyons2013}{Lyons \&
Beilock, 2013}). Some studies of SSS have systematically investigated
ordinality by investigating metrics of adjacent inducer-concurrent pairs
such as distances (\citeproc{ref-brang2010}{Brang et al., 2010}) or
angles (\citeproc{ref-eagleman2009a}{Eagleman, 2009}). Moreover,
synaesthetic number forms might be an ordinal construction
(\citeproc{ref-tang2008}{Tang et al., 2008}). Hence, ordinality of the
concurrent spatial-form might be particularly relevant considering all
the stimuli of a certain category (i.e.~weekdays or months).

A fourth limitation of consistency tests for SSS concerns the number
category. Indeed, theories from numerical cognition postulate a spatial
representation for numbers in the general population, with small numbers
on the left to large numbers on the right
(\citeproc{ref-dehaene1993}{Dehaene et al., 1993};
\citeproc{ref-hubbard2005a}{Hubbard et al., 2005}). This is sustained
experimentally by the the Spatial Numerical Association of Response Code
(SNARC) effect, where smaller numbers are responded faster with the left
hand and larger number with the right hand
(\citeproc{ref-dehaene1993}{Dehaene et al., 1993}). Hence
non-synaesthetic individuals could have an internalized representation
for numbers in space. Note however that the SNARC effect is not reliable
on an individual level (\citeproc{ref-cipora2019}{Cipora et al., 2019};
\citeproc{ref-ramos2026}{Ramos et al., 2026}). It is also debated
whether the SNARC originates from a semantic representational or later
response selection stage, such as stimulus-response
associations(\citeproc{ref-bassomoro2018}{Basso Moro et al., 2018};
\citeproc{ref-keus2005}{Keus \& Schwarz,
2005}).\pdfcomment[icon=Check,color={0 1 0}]{CS5: I am not sure what you mean by response versus representational level, maybe explain this quickly?}
In sum, consistent responses for numbers might also exist in
non-synaesthetic individuals.

In light of these limitations, we aim at optimizing the identification
of SSS from consistency tasks using geometrical characteristics of
spatial-forms from ordered stimuli or inducers.

\pdfcomment[icon=Check,color={0 0 1}]{NR6: Also add the problem of SNARC which is present in non-synesthetes and differentiate the phenomenon from synaesthesia -> this will give your research more impact, because it will also be interesting for the SNARC people. My recommendations do not have the be necessarily included in the same paragraph. For instance the information for consistency in GCS can be added somewhere further above and then further below you could write: in contrast to consistency in grapheme-colour synaesthesia, consistency in sequence-space synaesthesia has some limitations...}

\subsection{Present study}\label{present-study}

\pdfcomment[icon=Note,color={1 0 0}]{RL3: Several changes here too}

\textcolor{teal}{This registered report consists of two phases,} the
present \emph{phase I} is an exploratory study to find an optimal
consistency test to classify SSS and controls, on available aggregated
datasets. While the registered \emph{phase II} is a confirmatory study
of the most optimal consistency tests on data that has yet to be
collected.

In the present \emph{Phase I}, we merged four previously available
...
\pandocbounded{\includegraphics[keepaspectratio]{2.RegisteredReport_MS4_files/figure-pdf/fig-01-1.png}}
...
`ggplot2' v. 4.0.3 (\citeproc{ref-ggplot2}{Wickham, 2016}),
...
\citeproc{ref-ward2018}{Ward et al., 2018}). Participants were excluded
for having less than 4 coordinate points since polygons need at least
four coordinate (n = 2). Note therefore, that 31 participants of the
final sample did not have responses in all three categories b\textless{}
repetition cases
\pdfcomment[icon=Check,color={0 0 1}]{NR9: I am not able to understand this sentence. I find the X confusing.}.
From the final sample of N = 685, 396 were self-reported synaesthetes
and 289 self-reported controls, see Table~\ref{tbl-02}.
\pdfcomment[icon=Check,color={0 0 1}]{NR10: Based on self-reports?}

\begin{table}

{\caption{{Descriptives of each dataset}{\label{tbl-02}}}
\vspace{-20pt}}
...
& SSS & & & Controls & \\
& N & Age & & N & Age \\
(\citeproc{ref-rothen2016}{Rothen et al., 2016}) & 32 & 23.1 & & 37 &
28.2 \\
(\citeproc{ref-ward2022a}{Ward, 2022}) & 252 & 92.2 & & 213 & 19.9 \\
(\citeproc{ref-vanpetersen2020}{Van Petersen et al., 2020}) & 22 & 23.2
& & 21 & 21.6 \\
Ward (2026) & 90 & NA & & 18 & NA \\
\textbf{Total} & \textbf{396} & & & \textbf{289} & \\
\end{longtable}
}

\vspace{-20pt}
\noindent \emph{Note.} Ward~(2026) was provided by personal
communication
...
\pdfcomment[icon=Check,color={0 0 1}]{NR11: delete Professor -> also further above where you mention how you obtained the data.}
...
\pandocbounded{\includegraphics[keepaspectratio]{2.RegisteredReport_MS4_files/figure-pdf/fig-02-1.png}}

\vspace{-20pt}
\noindent \emph{Note.} Only~data from Ward is represented here. SSS =
...
(January to December) and 10 numbers (0 to 9). Each stimuli was repeated
three times. For all the data from Jamie Ward the stimuli were presented
in randomized order with the constraint that no stimulus was repeated
until all unique stimuli (N = 29) had been presented once. The median
display resolution was 1440 X 768, with a maximum of 2560 X 2025 and a
minimum of 308 X 149.

\subsection{Consistency methods and
analyses}\label{consistency-methods-and-analyses}

\pdfcomment[icon=Note,color={1 0 0}]{RL5: All this part has been restructured and refrased.}

\pdfcomment[icon=Check,color={0 0 1}]{NR12: General comments: The method reads well, but is hard to follow because the methods you are using are complex. It might help to always using the same structure for each method. First, provide a theoretical motivation for the method -> replicate the original study, exploring the possiblity that mappings are not random but orientied at topological validity. Second, describe the method. Third, give a concrete example. I do not think you have done a bad job, but the results are too difficult to follow, because the reader needs to remember the methods.}

\pdfcomment[icon=Check,color={0 0 1}]{NR13: We need to think carefully, what could be done to make it easier to follow. It might also help here to always apply the same structure. First, refere to descriptive statistics -> The descriptive statistics for comparing the ROC across different methods can be found in Figure X. Second, describe what you want to test / theoretical motivation -> in order to test X, we conducted Y analysis. Third, describe what the test revealed. -> It showed that A was significantly higher than B. The things you are doing are complex. Therefore the reader needs careful guidance.}

\pdfcomment[icon=Note,color={1 0 0}]{RL6: This whole section has been rewritten and partically restructured. The structure of each method is: theory, method, example}
...
\pdfcomment[icon=Check,color={0 0 1}]{NR14: the subtitle seems confusing, it is not clear what you mean with on the stimulus level.}
...
three repetitions
\pdfcomment[icon=Check,color={0 1 0}]{CS7: If you have done this, please ignore this comment (and sorry for having missed it), but if it’s not, somewhere in your manuscript you should metion that each grapheme had three repetitions in the studies you are talking about. And mention this before you say this like in the section above where you describe the procedure of the consistency tests})
formed by the \emph{x} and \emph{y} coordinates of the three repetitions
of each stimuli (i.e.~between the coordinates \emph{(x1, y1), (x2, y2),
(x3, y3)}).

\begin{equation}\protect\phantomsection\label{eq-area}{Area = \frac{|x1y2  +  x2y3  +  x3y1 \mathbin{-} x1y3 \mathbin{-}  x2y1 \mathbin{-} x3y2|}{2}}\end{equation}

\begin{equation}\protect\phantomsection\label{eq-perim}{\begin{split}
...
Each stimulus'
\pdfcomment[icon=Check,color={0 0 1}]{NR15: stimuli is plural -> does not seem to make sense here. Should be singular.}
...
recognized limitation of this method, is that controls clicking on the
same position can achieve artificially high consistencies, see for
example the first four rows of Figure~\ref{fig-03}.
...
\pandocbounded{\includegraphics[keepaspectratio]{fig-03.png}}
...
For the new methods for calculating consistency of this study
\pdfcomment[icon=Check,color={0 0 1}]{NR16: do you mean new method of calculating consistency?},
...
geometrical properties that are distinct than the ones of controls.
Later on we extract the same geometrical properties on segments and
polygons where the repetitions labels are permuted within each category
...
\pdfcomment[icon=Check,color={0 0 1}]{NR17: for the above paragraph -> not clear what for each repetition means. do you mean trial 1 of each inducer of a given participant?}
...
\pdfcomment[icon=Check,color={0 0 1}]{NR18: not clear what you mean by conditions? -> stimulus type / category?}
and repetitions.

\pdfcomment[icon=Check,color={0 1 0}]{CS8: I am not sure if you should jump back to Figure 1 over here. If it is more important to have the information here maybe move the figure after this text instead of at the beginning. This would also mean that you remove the reference to figure 1 at the beginning and change it to figure 3 (or whatever chronological order it is in over here). I say this because right now you are jumping back and forth.}

\pdfcomment[icon=Note,color={1 0 0}]{RL7: I would argue the methodological design is on a thoeretical level, but I also refer it here as a concrete example.}
...
\pdfcomment[icon=Check,color={0 0 1}]{NR19: each repetition or each of the repetitions?}
...
We assessed topological properties using geometric validation test. We
...
\pdfcomment[icon=Check,color={0 0 1}]{NR20: there is some repetition}
...
\pdfcomment[icon=Check,color={0 0 1}]{NR21: sometimes you seem to call it conditions and now categories. Be consistent with terminology.}
and 3 repetitions, hence 9 space-forms or polygons.

For the example of a participant, having all weekday's space-forms
...
\pdfcomment[icon=Check,color={0 0 1}]{NR22: the example helps a lot, maybe add a concrete example for all methods.}
...
\begin{equation}\protect\phantomsection\label{eq-PP}{P - P\ score = \frac{4\pi \cdot Area}{Perimeter^2}}\end{equation}
...
\subsubsection{Permutation-based feature
...
(repetition 1), Tuesday (repetition 3), Wednesday (repetition 2), etc.,
...
\begin{equation}\protect\phantomsection\label{eq-DP}{ 
DP = \frac{\sqrt{3}}{\pi} (\log(\frac{sensitivity}{1−sensitivity}) + \log(\frac{specificity}{1−specificity}))  }\end{equation}
...
\begin{equation}\protect\phantomsection\label{eq-D}{D = \frac{AUC_1-AUC_2}{SD}}\end{equation}

Therefore we statistically compared all the AUC of each method
\pdfcomment[icon=Check,color={0 0 1}]{NR23: methods or method?} as the
paired difference to the method with the highest AUC. P-values were
adjusted with false discovery rate over all paired comparisons.

\section{Results - phase I}\label{sec-results}

\pdfcomment[icon=Check,color={0 1 0}]{CS9: I think this section is a little confusing to read because you talk about ROC and AUC, but then also DP. But then you go back to talking about ROC and AUC and it is a little unclear how DP fits into the story.}

\pdfcomment[icon=Check,color={0 0 1}]{NR24: The results section until the discussion of the present study is quite hard to follow. It requires simplification}

\pdfcomment[icon=Check,color={0 1 0}]{CS10: I am a bit confused in the results section, maybe this is just me. But, I was wondering if it would be good to structure the results section in the same way you have spoken about the different methods above? Or perhaps it needs to be made clearer that descriptives and ROC analyses were cnoducted on the methods above because right now, it is not very clear.}

\pdfcomment[icon=Note,color={1 0 0}]{RL9: I have made this section shorter and tried to streamline the message}

\subsection{Reproduced consistency test and
descriptives}\label{reproduced-consistency-test-and-descriptives}

Regarding the current consistency test (\citeproc{ref-rothen2016}{Rothen
et al., 2016}), the available datasets resulted in descriptively
different averages for SSS, see Table~\ref{tbl-04}. The triangle area
(in \% screen size) for SSS was surprisingly larger in the present
aggregated dataset 0.26\% (\emph{SD = 0.52\%}) than in previous studies
studies: 0.14\% (\emph{SD = 0.13\%}) in Rothen et al.
(\citeproc{ref-rothen2016}{2016}) and 0.14\% (\emph{SD = 0.17}\%) in
Ward et al. (\citeproc{ref-ward2018}{2018}). Descriptives break down by
dataset (see \hyperref[apx-SM2byds]{Appendix~B}) suggest that the
dataset with the largest number of participants
...
cannot directly be compared to each other. Hence the rightmost columns
...
the need for additional analyses from \emph{phase II}. It also stresses
out the use of methods that are either standardized or that do not rely
on metrics that depend on the experimental settings.

\subsection{ROC analyses}\label{roc-analyses}

Permuted validity and the standardized perimeter led to the highest AUC
and did not statistically differ between each other (\emph{D} = 1.4,
\emph{p =} 0.16). The results of the ROC analyses are summarized in
Table~\ref{tbl-03} and Figure~\ref{fig-05}, where consistency methods
are sorted by the highest Area Under the Curve (AUC).
...
\pandocbounded{\includegraphics[keepaspectratio]{2.RegisteredReport_MS4_files/figure-pdf/fig-05-1.png}}
...
{\caption{{ROC descriptives the consistency tests}{\label{tbl-03}}}
\vspace{-20pt}}
...
The Polsby-Popper method, while having the second highest AUC (i.e.
78.96), was statistically different compared to the permuted valid score
(\emph{D} = 2.08, \emph{p} = \textless{} .05). Hence,
counter-intuitively, although the Polsby-Popper showed descriptively
larger AUC than the standardized perimeter, it did not significantly
differ from the permuted validity score, see Table~\ref{tbl-03}. We
conducted further analyses on sensitivity and specificity to understand
this result. Comparing sensitivity and specificity of the permuted
validity to the Polsby-Popper revealed no significant differences in
specificity (i.e.~at a sensitivity of 80\%, \emph{D =} 0, \emph{p} =
\emph{n.s.}) nor sensitivity (i.e.~at a specificity of 80\%, \emph{Z =}
1.02, \emph{p} = \emph{n.s.}). We then compared the Polsby-Popper to the
standardized perimeter. We found higher specificity for standardized
perimeter than the Polsby-Popper method (at a sensitivity of 80 \%,
\emph{D =} 2.04, \emph{p} = 0.04). See also Figure~\ref{fig-apx01} for
the density plot's distribution across self-reported SSS and controls.
...
calculated on the space-form polygon's area and perimeter using
Equation~\ref{eq-PP}.

In sum, the permuted validity, Polsby-Popper and standardized perimeter
test, are mostly comparatively performant. However on a more
fine-grained conclusion is that, in terms of AUC, Permuted validity and
Polsby-Popper lead to better performances. However the standardized
perimeter is more sensible compared to the Polsby-Popper, hence
perimeter is more recommended to correctly test for SSS.
...
\pdfcomment[icon=Note,color={1 0 0}]{RL10: This is new:}We developed an
...
and inconsistent synesthetes (see \citeproc{ref-root2025}{Root et al.,
2025}; \citeproc{ref-eagleman2007}{Eagleman et al., 2007}), see
...
\pdfcomment[icon=Check,color={0 1 0}]{CS11: I don’t think you need any of the sections from here onwards for a pre-registration/registered report However, saying this, I think you can move some bits of the discussion section into the results section.I do not think you should discuss limitations right now}
...
In \emph{phase II}, we will attempt at validating these criteria on a
...
\bibitem[\citeproctext]{ref-tang2008}
Tang, J., Ward, J., \& Butterworth, B. (2008). Number forms in the
brain. \emph{Journal of Cognitive Neuroscience}, \emph{20}(9),
1547--1556. \url{https://doi.org/10.1162/jocn.2008.20120}
...
\pandocbounded{\includegraphics[keepaspectratio]{2.RegisteredReport_MS4_files/figure-pdf/fig-apx01-1.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{2.RegisteredReport_MS4_files/figure-pdf/fig-apx02-1.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{2.RegisteredReport_MS4_files/figure-pdf/fig-apx03-1.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{2.RegisteredReport_MS4_files/figure-pdf/fig-apx04-1.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{2.RegisteredReport_MS4_files/figure-pdf/fig-apx05-1.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{2.RegisteredReport_MS4_files/figure-pdf/fig-apx06-1.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{2.RegisteredReport_MS4_files/figure-pdf/fig-apx07-1.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{2.RegisteredReport_MS4_files/figure-pdf/fig-apx08-1.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{2.RegisteredReport_MS4_files/figure-pdf/fig-apx09-1.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{2.RegisteredReport_MS4_files/figure-pdf/fig-apx10-1.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{2.RegisteredReport_MS4_files/figure-pdf/fig-apx11-1.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{2.RegisteredReport_MS4_files/figure-pdf/fig-apx12-1.png}}
