...
individuals with SSS are consistent and automatic. Existing methods for
classifying SSS and controls based on consistency tests have some
limitations. The aim of the present study is to optimize consistency
methods from aggregated and already available datasets by adopting.Then,
in a future study, we aim to test and compare the optimal methods on
data that has yet to be collected.
...
month), see Figure~\ref{fig-myplot01}.
...
condition (see Section~\ref{sec-permutation-based-feature-extraction}).
...
extracted the geometrical feature from x,y coordinates of stimuli from
the same categories in non-chronological order. Specifically, for each
