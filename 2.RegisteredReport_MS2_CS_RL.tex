...
of spatial-forms. This preregistered study consists of a present and
future analyses. In the present analyses, we optimise SSS diagnostics
from consistency test by extracting novel tests at the spatial-form
level. To do this, we first evaluate classification performance of old
and new methods in a large (N = 685) aggregated sample from four
available and independent datasets. We then harness a geographic toolbox
to extract metrics from the spatial-forms level. Finally, receiver
operating characteristic analyses are conducted to assess the
discriminative performances of each method. The results show that
permuted topological validity of spatial-forms and the perimeter between
stimuli performs equally well in detecting SSS. In a future study, we
will examine predictive validity of the selected methods on an
independent dataset that has yet to be collected. Findings from the
present study highlight the relevance of topological principles for
sequence-space representations and aim to optimise the ability of
...
\textcolor{red}{[NR, comment 3: add reference / it is not clear that the shapes differ from person to person -> they are idiosyncractic]}.
Existing methods for classifying SSS and controls based on consistency
tests have some limitations
\textcolor{red}{[NR, comment 4: add references to some existing tests]}.
The aim of the present study is to optimize consistency methods from
aggregated and already available datasets by adopting
\textcolor{red}{[NR, comment 5: adopting what?]}. Then, in a future
study, we aim to test and compare the optimal methods on data that has
yet to be collected.
\textcolor{red}{[NR, comment 6: The sturcture of the opening paragraph is very nice! However, it is not clear that Study 1 and Study 2 will both be in the same paper, while Study 1 is exploratory and Study 2 based on registration of the results from the first study confirmatory. I think, you must make clear that Study 2 is why we submit this manuscript as registered report.]}
...
synaesthesia
\textcolor{red}{[NR, comment 7: it is grapheme-colour synaesthesia -> inducer is always called first.]},
where a grapheme \emph{inducer} triggers a \emph{concurrent} colour
(i.e.~``A'' seen as red), co-occurs at 71\% to 76\% with SSS
(\citeproc{ref-sagiv2006}{Sagiv et al., 2006}). Nevertheless, across
different synaesthetic subtypes, self-reported SSS tend to cluster
together (\citeproc{ref-ward2022}{Ward \& Simner, 2022}), indicating a
degree of internal homogeneity and justifying the study of it as a
distinct phenomenon. Spatial-forms are idiosyncratic, which results in
considerable heterogeneity in how this phenomenon is manifested across
individuals. One source of heterogeneity is given by dimensionality:
some SSS experiences can involve three-dimensional (3D) and
...
not informed about the ordinality between stimuli (e.g.~1 \texttt{→} 2
\texttt{→} 3, etc.). Here, we aim at optimizing the identification of
SSS from consistency tasks using geometrical characteristics of
...
an indicator of the consistent response characteristic of SSS.
\textcolor{red}{[NR, comment 10: Add explanation -> because synaesthetes can rely on experiences, but non-synaesthetes must rely on memory]}
...
\textcolor{red}{[NR, comment 11: Do these limitations apply in general or only to SSS consistency tests?]}
Some tests may favour certain spatial-forms
(\citeproc{ref-ward2018}{Ward et al., 2018}) in particular those with
linear or highly regular spatial layouts, i.e.~elliptical patterns for
months(\citeproc{ref-brang2010}{Brang et al., 2010};
\citeproc{ref-eagleman2009a}{Eagleman, 2009};
...
analyses. In the present \emph{Phase I}, we merge four previously
available datasets using the same task on both SSS and control groups
...
level, see Figure~\ref{fig-myplot00}. All the consistency tests are then
compared using ROC analyses on their correct
\textcolor{red}{[NR, comment 14: better -> optimizing based on self-report]}
classification performances of self-reported SSS and control groups.

In a future study
\textcolor{red}{[NR, comment 15: could be called Phase II. In a future is not so clear whether it will be part of this manuscript or something entirely new. It must be made clear that it will be part of this manuscript. You could also state that this study consists of two phases: phase I is exploratory and based on existing data and phase II is confirmatory and based on to be collected data.]},
...
1.19.0.1 (\citeproc{ref-pROC}{Robin et al., 2011}), `sf' v. 1.1.0
(\citeproc{ref-sf}{Pebesma \& Bivand, 2023}) and for visualization
`ggplot2' v. 4.0.2 (\citeproc{ref-ggplot2}{Wickham, 2016}),
`ggVennDiagram' v. 1.5.7 (\citeproc{ref-ggVennDiagram}{Gao \& Dusa,
2025}).
...
gently \textcolor{red}{[NR, comment 16: remove gently]}provided by
private communications with Professor Ward. To match the other datasets,
stimuli of the weekdays and months categories were translated from Dutch
to English in the dataset from (\citeproc{ref-vanpetersen2020}{Van
Petersen et al., 2020}). For the number category we included only digits
from 0 to 9 that where presented across all datasets (i.e.~excluding the
stimuli ``50'' and ``100'' for numbers in Van Petersen et al.
(\citeproc{ref-vanpetersen2020}{2020})).
...
conditions and repetitions, causing the depletion (n = 2) participants
...
\textcolor{red}{[NR, comment 17: I am not able to understand this sentence. I find the X confusing.]}.
From the final sample of N = 685, 396 were synaesthetes and 289
controls, see Table~\ref{tbl-mytable01} and Table~\ref{tbl-mytable02}.
\textcolor{red}{[NR, comment 18: Based on self-reports?]}
...
\textcolor{red}{[NR, comment 19: delete Professor -> also further above where you mention how you obtained the data.]}
Ward (i.e.~a majority of 573 participants). We described the
self-reported profiles for the stimulus categories used in the
consistency test (i.e.~number, weekdays and month), see
Figure~\ref{fig-myplot01}.
...
\pandocbounded{\includegraphics[keepaspectratio]{2.RegisteredReport_MS2_CS_RL_files/figure-pdf/fig-myplot01-1.pdf}}
...
\textcolor{red}{[NR, comment 20: the subtitle seems confusing, it is not clear what you mean with on the stimulus level.]}

We reproduced the tests
\textcolor{red}{[NR, comment 21: do you mean the results?]} extracted
from consistency tasks
(\textcolor{red}{[NR, comment 22: do you mean the studies?]} found in
the literature such as area and perimeter between repetitions
...
Each stimuli's
\textcolor{red}{[NR, comment 23: stimuli is plural -> does not seem to make sense here. Should be singular.]}
...
(\emph{z-score}). These methods compute consistency metrics at the
stimulus level
\textcolor{red}{[NR, comment 24: for each participant separately?]}, the
rationale is that consistent responses would show less variability
between repetitions. For example, smaller area or perimeter of the
triangle formed indicate more variability hence more consistent
responses between the three repetitions.
...
\textcolor{red}{[NR, comment 25: do you mean new method of calculating consistency?]},
...
described in Galton (\citeproc{ref-galton1880}{1880}).

\textcolor{red}{[NR, comment 26: for the above paragraph -> not clear what for each repetition means. do you mean trial 1 of each inducer of a given participant?]}

We harnessed a geospatial analysis, the `sf' package version 1.1.0
...
segments and polygon by conditions
\textcolor{red}{[NR, comment 27: not clear what you mean by conditions? -> stimulus type / category?]}
...
(i.e.~as segments or polygon) they remain consistent \emph{between}
...
\textcolor{red}{[NR, comment 28: can you give a concrete example?]}

The space-form-based methods hence rely on the chronologically
\textcolor{red}{[NR, comment 29: sequentally?]} ordered repetitions. For
...
order of stimulus presentation. Hence, we also calculated two of the
following methods by permuting the repetitions within each condition
(see Section~\ref{sec-permutation-based-feature-extraction}).
...
computed separately for each repetitions
\textcolor{red}{[NR, comment 30: each repetition or each of the repetitions?]}
and conditions and averaged per participants.
...
self-touch. \textcolor{red}{[NR, comment 31: there is some repetition]}
...
\textcolor{red}{[NR, comment 32: sometimes you seem to call it conditions and now categories. Be consistent with terminology.]}
...
\textcolor{red}{[NR, comment 33: the example helps a lot, maybe add a concrete example for all methods.]}
...
\textcolor{red}{[NR, comment 34: I wonder whether chronological is the right word for this. How about trial order, sequential order, etc.]}
order, we shuffled the repetition order between stimulus presentation
(i.e.~Monday (repetition 2) \texttt{→} Tuesday (repetition 3) \texttt{→}
Wednesday (repetition 1), ect) and extracted the geometrical feature
from x,y coordinates of stimuli from the same categories in
non-chronological order. Specifically, for each participant and each
category, we randomized the response repetition order. For example
segments for weekdays were constructed with the x,y coordinates randomly
...
\textcolor{red}{[NR, comment 35: methods or method?]} as the difference
to the highest AUC method.
...
\pandocbounded{\includegraphics[keepaspectratio]{2.RegisteredReport_MS2_CS_RL_files/figure-pdf/fig-myplot00-1.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{2.RegisteredReport_MS2_CS_RL_files/figure-pdf/fig-myplot2-1.pdf}}
...
\textcolor{red}{[NR, comment 36: The results section until the discussion of the present study is quite hard to follow. It requires simplification]}
...
(\emph{D} = 2.2 , \emph{p} \textless{} .05), it led to similar AUC than
the standardized perimeter (\emph{D} = 1.47, \emph{p} \textless{} .05).
This might be explained by the relatively lower sensitivity of 59.7
compared to the Perimeter (72.54) and the permuted validity tests
(70.78). The Polsby-Popper test also resulted in the largest
discrimination power of all the tests (2.29 ) which is explained by the
high specificity of 87.54, meaning this test outperformed the others in
correctly classifying SSS, but was relatively relaxed in classifying
controls as SSS. Additionally, the permuted validity led to
significantly higher AUC than the non permuted one (D = 3.14, \emph{p}
\textless{} .05).
...
higher specificity than perimeter (78.55 vs. 75.78) indicating more
accurate rejection of controls (i.e.~less false positives), but higher
sensitivity (70.78 vs. 72.54), indicating less accurate detection of SSS
...
\citeproc{ref-root2025}{Root et al., 2025}). We found \textasciitilde6\%
of the full sample's synesthetes to be consistently classified as
controls (i.e. consistent false negatives) by the five methods with the
highest AUC, see , see Figure~\ref{fig-myplotapx07} and
...
v. 0.12.6 (\citeproc{ref-ggalluvial}{Brunson \& Read, 2023}), `corrplot'
...
\pandocbounded{\includegraphics[keepaspectratio]{2.RegisteredReport_MS2_CS_RL_files/figure-pdf/fig-apx01-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{2.RegisteredReport_MS2_CS_RL_files/figure-pdf/fig-myplotapx02-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{2.RegisteredReport_MS2_CS_RL_files/figure-pdf/fig-apx08-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{2.RegisteredReport_MS2_CS_RL_files/figure-pdf/fig-myplotapx03-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{2.RegisteredReport_MS2_CS_RL_files/figure-pdf/fig-myplotapx04-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{2.RegisteredReport_MS2_CS_RL_files/figure-pdf/fig-myplotapx06-1.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{2.RegisteredReport_MS2_CS_RL_files/figure-pdf/fig-myplotapx07-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{2.RegisteredReport_MS2_CS_RL_files/figure-pdf/fig-myplotapx08-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{2.RegisteredReport_MS2_CS_RL_files/figure-pdf/fig-myplotapx09-1.pdf}}
