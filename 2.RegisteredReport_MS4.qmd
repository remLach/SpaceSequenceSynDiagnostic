  ...
  - name: Nicolas Rothen
    ...
    affiliations:
      - name: UniDistance Suisse
        ...
        region: Valais
        postal-code: 3900   
abstract: "Sequence-space synesthesia is a perceptual condition in which ordinal sequences (e.g., numbers, days of the week, or months of the year) are experienced as occupying specific spatial locations. This perceptual condition is identified using self-reports (e.g. questionnaires) and behavioural consistency test methods. Existing consistency methods measure the spatial distance between responses to repeated stimuli, but ignore the ordinal and geometric properties of spatial-forms. This registered report consists of two phases In *phase I*, we optimise SSS diagnostics from consistency test by extracting novel tests at the spatial-form level. In this exploratory phase we evaluated classification performance of old and new methods in a large (N = 685) aggregated sample from four available and independent datasets. We then harness a geographic toolbox to extract metrics from the spatial-forms level. Finally, receiver operating characteristic analyses are conducted to assess the discriminative performances of each method. The results show that permuted topological validity of spatial-forms and the perimeter between stimuli performs equally well in detecting SSS. In confirmatory *phase II*, we will examine predictive validity of the selected methods on an independent dataset that has yet to be collected. Findings from *phase I* highlight the relevance of topological principles for sequence-space representations and aim to optimise the ability of consistency tests to detect SSS."
...
be complex and can have variable geometric forms. These geometric forms
are idiosyncratic and might be different for each category (i.e.
number-forms weekdays and months), with forms such as ellipses, zig-zags
or curbed lines [@galton1880; @flournoy1893]. However, the spatial-forms
of individuals with SSS are consistent and automatic [@seron1992;
@deroy2013]. Existing methods for classifying SSS and controls based on
consistency tests have some limitations [@brang2010; @rothen2016;
@ward2022; @ward2018; @eagleman2007; @sagiv2006; @baron-cohen1987].
Therefore, in an exploratory *phase I* study, we optimized the
consistency methods from aggregated and already available datasets.
Then, in a confirmatory *phase II* registered report study, we aim to
test and compare the optimal methods on data that has yet to be
collected.
...
```{=html}
<!--# SSS frequently co-occurs with other subtypes of synaesthesia
`r cs("I think this kind of comes out of nowhere, it might be better to distinguish between the two points: 1. SSS idionsyncratic.2. It co-occurs with other types of synesthesia.", done = 1)`.
Grapheme-colour synaesthesia, where a grapheme *inducer* triggers a
*concurrent* colour (i.e. “A” seen as red), co-occurs at 71% to 76% with
SSS [@sagiv2006]. Nevertheless, across different synaesthetic subtypes,
self-reported SSS tend to cluster together [@ward2022], indicating a
degree of internal homogeneity and justifying the study of it as a
distinct phenomenon. -->
```

Spatial-forms are idiosyncratic,
`r cs("I would mention this where you first say that SSS is idionsyncratic (the first sentence of this paragraph).", done =1)`
...
and two-dimensional (2D) arrangements [@eagleman2009a; @price2013].
Another source of heterogeneity is the reference frame, for example, the
spatial-forms might take place in an external space around the body
(*i.e.* projector) or in an egocentric internal space (*i.e.*
associator) [@dixon2004; @smilek2007]. Further variability can be
explained by temporal-spatial properties and mental-manipulation of
spatial-forms such as “zooming” in and out or rotating or shifting
perspectives at will [@gould2014; @jarick2009]. Lastly, the shape,
...
months [@eagleman2009a].
`r track("Despite this heterogeneity, SSS responses are consistent, meaning that the spatial position to the same inducers should be constant in time.")`

## Consistency test {#sec-consistency-tests}

`r rl("I changed the order in the paragraphs so that I start with how synaesthesia is diagnosed in general (self-reports & consistency). Then define consistency and how it is applied to GCS. Finally how it is tested in SSS.")`

`r cs("Overall, this section is great, but I wonder if it would be better to first describe consistency tests more generally and only later say how it is applied for SSS. I say this because at the moment, you talk about it in relation to SSS first, then GCS, and then back to SSS. Also, I would refer to grapheme-color synesthesia, not color-grapheme synesthesia. I think grapheme-color is the correct way to say it.", done = 1)`
...
@rothen2016]. An individual is identified as a synaesthete based on the
variability between repeated responses to the same inducer or stimuli:
less variable responses being an indicator of the consistent responses
characteristic of synaesthesia.
`r track("This is because synaesthetes can respond from their perceptual experience while non-synesthetes can only be consistent by relying on a memorization strategy")`
...
tasks. However, consistency tests for colour-grapheme synaesthesia
generally show better performance than the correspondent tests used for
SSS (see @tbl-01).

|             |                              |                             |
|:-----------:|:----------------------------:|:---------------------------:|
|             | Grapheme-Colour Synaesthesia | Sequence-space Synaesthesia |
|             |        [@rothen2013a]        |        [@rothen2016]        |
|     AUC     |             93%              |             76%             |
| Sensitivity |             90%              |             88%             |
| Specificity |             94%              |             77%             |

: Consistency tasks performances for grapheme-colour and sequence space
synaesthesia {#tbl-01 apa-note="AUC = Area Under the Curve"}
...
*inducer*-*concurrent* associations). @brang2010 for example evaluated
...
`r rl("this whole section has been restructured and/or rewritten")`

`r cs("Would it be useful to compare SSS consistency tests to the GCS one as far limitaitons go? I am not sure though", done = 1)`
...
high consistencies[@rothen2016]. This problem has particularly been
identified for SSS. Indeed, exclusion via visual inspection of these
responses for SSS lead to improved classification performances using the
area test (i.e. Area Under the Curve = 85% Sensitivity = 87%;
Specificity = 81%, , see @rothen2016).
`r nr("Do these limitations apply in general or only to SSS consistency tests?", done = 1)`

```{=html}
<!--# (i.e. lead to more accurate classifications with an an Area Under
the Curve of 93%). For example, existing consistency tests for
Colour-grapheme lead to , which corresponds to the probability of the
test to correctly classify synesthetes from non synaesthetes
(Sensitivity = 90%, Specificity = 94%, see @rothen2013a). In contrast,
consistency tests for SSS lead to an Area Under the Curve of 76%
(Sensitivity = 88%, Specificity = 77%, see @rothen2016).
...
used[@ward2018]. In particular for spatial-forms with linear or highly
...
angles [@eagleman2009a]. Moreover, synaesthetic number forms might be an
ordinal construction [@tang2008]. Hence, ordinality of the concurrent
spatial-form might be particularly relevant considering all the stimuli
of a certain category (i.e. weekdays or months).
...
on the left to large numbers on the right [@dehaene1993; @hubbard2005a].
This is sustained experimentally by the the Spatial Numerical
Association of Response Code (SNARC) effect, where smaller numbers are
responded faster with the left hand and larger number with the right
hand [@dehaene1993]. Hence non-synaesthetic individuals could have an
internalized representation for numbers in space. Note however that the
SNARC effect is not reliable on an individual level [@cipora2019;
@ramos2026]. It is also debated whether the SNARC originates from a
semantic representational or later response selection stage, such as
stimulus-response associations[@bassomoro2018;
@keus2005].`r cs("I am not sure what you mean by response versus representational level, maybe explain this quickly?", done = 1)`
...
`r track("This registered report consists of two phases,")` the present
*phase I* is an exploratory study to find an optimal consistency test to
classify SSS and controls, on available aggregated datasets. While the
registered *phase II* is a confirmatory study of the most optimal
consistency tests on data that has yet to be collected.

In the present *Phase I*, we merged four previously available datasets
...
segments and polygons. These novel tests were designed to take advantage
...
consistency tests were then optimized using ROC analyses based on
self-reported
...
tmp2 <- ds %>%
  ...
  filter(row_number() == 1) %>%
  reframe(ntrials = length(x)) %>%
  ungroup() %>%
  group_by(ID) %>%
  reframe(nCond = sum(ntrials)/3)
...
@ward2018]. Participants were excluded for having less than 4 coordinate
points since polygons need at least four coordinate (n = `r N2-N3`).
...
`r sum(ds_Q$group == "Ctl")` self-reported controls, see @tbl-02.
`r nr("Based on self-reports?", done =1)`

|                    |         |      |     |          |      |
|:------------------:|:-------:|:----:|-----|:--------:|:----:|
|                    |   SSS   |      |     | Controls |      |
|                    |    N    | Age  |     |    N     | Age  |
|   [@rothen2016]    |   32    | 23.1 |     |    37    | 28.2 |
|    [@ward2022a]    |   252   | 92.2 |     |   213    | 19.9 |
| [@vanpetersen2020] |   22    | 23.2 |     |    21    | 21.6 |
|    Ward (2026)     |   90    |  NA  |     |    18    |  NA  |
|     **Total**      | **396** |      |     | **289**  |      |

: Descriptives of each dataset {#tbl-02 apa-note="Ward (2026)
was provided by personal communication"}
...
#| apa-note: "Only data from Ward is represented here. SSS = Sequence-space synaesthesia"
...
display resolution was `r median(ds$width)` X `r median(ds$height)`,
with a maximum of `r max(ds$width)` X `r max(ds$height)` and a minimum
of `r min(ds$width)` X `r min(ds$height)`.

## Consistency methods and analyses
...
calculated from the triangle (i.e. given three repetitions
`r cs("If you have done this, please ignore this comment (and sorry for having missed it), but if it’s not, somewhere in your manuscript you should metion that each grapheme had three repetitions in the studies you are talking about. And mention this before you say this like in the section above where you describe the procedure of the consistency tests", done = 1)`)
formed by the *x* and *y* coordinates of the three repetitions of each
stimuli (i.e. between the coordinates *(x1, y1), (x2, y2), (x3, y3)*).

$$Area = \frac{|x1y2  +  x2y3  +  x3y1 \mathbin{-} x1y3 \mathbin{-}  x2y1 \mathbin{-} x3y2|}{2}$$ {#eq-area}
...
example the first four rows of @fig-03.
...
the ones of controls. Later on we extract the same geometrical
properties on segments and polygons where the repetitions labels are
permuted within each category (i.e. Monday (repetition 2) `→` Tuesday
(repetition 1) `→` Wednesday (repetition 3), ect.). The rationale here
is that spatial-forms should have the same geometrical properties
independently from coordinate repetition order of the responses during
the experiment.
...
`r cs("I am not sure if you should jump back to Figure 1 over here. If it is more important to have the information here maybe move the figure after this text instead of at the beginning. This would also mean that you remove the reference to figure 1 at the beginning and change it to figure 3 (or whatever chronological order it is in over here). I say this because right now you are jumping back and forth.", done = 1)`

`r rl("I would argue the methodological design is on a thoeretical level, but I also refer it here as a concrete example.")`
...
do not cross (3) boundary rings may only touch points tangentially (4)
rings that define holes are contained within the exterior ring (5) the
polygon rings must not split the polygon [@postgis].
...
district @polsby1991. In the context of geopolitics, gerrymandering is a
politically motivated strategy of redefining electoral district's
boundaries to advantage a political party. As a consequence, the
...
$$P - P\ score = \frac{4\pi \cdot Area}{Perimeter^2}$$ {#eq-PP}
...
presented, see @fig-01. Hence, we re-calculated the three most
performant tests (i.e. leading to higher Area under the Curve) by
...
$$ 
DP = \frac{\sqrt{3}}{\pi} (\log(\frac{sensitivity}{1−sensitivity}) + \log(\frac{specificity}{1−specificity}))  $$ {#eq-DP}
...
$$D = \frac{AUC_1-AUC_2}{SD}$$ {#eq-D}

Therefore we statistically compared all the AUC of each method
`r nr("methods or method?", done = 1)` as the paired difference to the
method with the highest AUC. P-values were adjusted with false discovery
rate over all paired comparisons.
...
tmp_perID2 <- ds %>% 
    ungroup() %>% 
    group_by(ID) %>% 
    reframe(Area_perc = sum(rep_area, na.rm = TRUE)/(sum(!is.na(rep_area)))*100, Screen_area = unique(Screen_area))  %>%
...
tmp_perID2 <- ds %>% ungroup() %>% 
  group_by(ID) %>% 
  reframe(Area_zs = mean(rep_area_zs, na.rm = TRUE))  %>%
  select(ID, Area_zs)
...
tmp_ID <- ds %>%
  ...
  reframe(Perimeter_perc = mean(triangle_perim, na.rm = TRUE)) %>%
...
tmp_ID <- ds %>%
  ungroup() %>% group_by(ID,group) %>%
  reframe(Perimeter_zs = mean(triangle_perim_zs, na.rm = TRUE)) %>%
...
ds_Q <- right_join(ds_Q,
                   ds_poly %>% st_drop_geometry() %>% ungroup() %>% group_by(ID) %>% reframe(pp_M = mean(pp, na.rm = TRUE)) %>% select(ID, pp_M),
                   by = "ID")
...
Comp_ROC <- function(data, group_col, feature, ID, Ord){
  ...
  Descr_table <- data %>%
    ...
    reframe(n = length(unique(!!sym(ID))), Mean = mean(!!sym(feature), na.rm = TRUE), SD = sd(!!sym(feature), na.rm = TRUE))
...
# Since random sampling is involved in bootstrapping, to make it reproductible:
set.seed(42)
...
statistic <- sapply(seq_along(ROC_listed), function(i) {
  ...
  roc.test(toCompare_roc, ROC_listed[[i]],  boot.n=1000,method = "bootstrap", paired = TRUE)$statistic
})

pvals <- sapply(seq_along(ROC_listed), function(i) {
  if (i == sel_auc) return(NA)
  roc.test(toCompare_roc, ROC_listed[[i]],  boot.n=1000,method = "bootstrap", paired = TRUE)$p.value
})
...
# Figure is exported and reimported to better position in text
...
header <- plot_grid(
  ...
  ggdraw() + draw_label("Classified as most control", fontface = "bold",fontfamily = "serif", hjust = 0.2),
...
subheader <- plot_grid(
  ...
  ggdraw() + draw_label("Month     Weekday      Number", fontfamily = "serif", size = 10, hjust = 0.25),
  ggdraw() + draw_label("Month     Weekday      Number",fontfamily = "serif", size = 10, hjust = -0.25),
  ncol = 4, rel_widths = c(0.5,1,1)
)
...
# List ID's with data for all 3 conds:
ID3Cond <- ds %>% 
  group_by(ID) %>% 
  reframe(nCond = length(unique(Cond))) %>% 
  filter(nCond == 3) %>%
  pull(ID)

# To present by AUC informed ranking of the methods:
Methods_byAUC <- c("isValid_perm_M", "pp_M",  "Perimeter_zs",  "isValidStruct_M", "pp_perm_M", "SelfInter","areaPoly_GA","isSimple_GA","Area_zs", "perimPoly", "Perimeter_perc",  "Area_perc")

# We (rendomly) sample the participants, otherwise would have a diferent figure each time.
set.seed(42)
for(test_i in 1:length(Methods_byAUC)){ # keep Test_names for another oreder
  test_here <- Methods_byAUC[test_i]
...
`r cs("I am a bit confused in the results section, maybe this is just me. But, I was wondering if it would be good to structure the results section in the same way you have spoken about the different methods above? Or perhaps it needs to be made clearer that descriptives and ROC analyses were cnoducted on the methods above because right now, it is not very clear.", done = 1)`

`r rl("I have made this section shorter and tried to streamline the message")`

## Reproduced consistency test and descriptives

Regarding the current consistency test [@rothen2016], the available
datasets resulted in descriptively different averages for SSS, see
@tbl-04. The triangle area (in % screen size) for SSS was surprisingly
larger in the present aggregated dataset
...
other. Hence the rightmost columns of @tbl-04 also reports standardized
...
tmp <- tmp[sort(All_ROC$AUC,  decreasing = TRUE, index.return = TRUE)$ix,]
...
gt(tbl_out) |>
  ...
  tab_footnote(
    footnote = "Note: mean (standard deviation) of all the consistency tests. The two rightmost columns display the same descriptives in z-score transformed values, enabling comparison across different metrics. Z-scores were calculated for all participants, separately for each test.") 

rm(tmp)
# Sanity Check: The descriptives for Area_perc calculated here are the ~same as in Ward
## Here:
# ds_Q %>% group_by(group) %>% filter(dataSource %in% "Ward") %>% reframe(M = mean(Area_perc, na.rm = TRUE), sd = sd(Area_perc, na.rm = TRUE))
## Ward
# ds_Q %>% group_by(group) %>% filter(dataSource %in% "Ward") %>% reframe(M = mean(consistency_score, na.rm = TRUE), sd = sd(consistency_score, na.rm = TRUE))
```

In sum, the descriptives stress out the variability across datasets and
the need for additional analyses from *phase II*. It also stresses out
the use of methods that are either standardized or that do not rely on
metrics that depend on the experimental settings.

## ROC analyses

Permuted validity and the standardized perimeter led to the highest AUC
and did not statistically differ between each other (*D* =
...
where consistency methods are sorted by the highest Area Under the Curve
(AUC).
...
ggroc(ROC_listed) +
  ...
  annotate(geom = "curve", x = 72.25, y = 75.78 , xend = 42, yend = 51, 
           curvature = -.3, arrow = arrow(length = unit(2, "mm")), color = "#6A33C2" , alpha = 0.5) +
  annotate(geom = "text", x = 42, y = 51, label = "Perimeter", 
           hjust = "left", color = "#6A33C2" , size = 3) 
...
```{r}
#| label: tbl-03 
#| tbl-cap: "ROC descriptives the consistency tests"
#| ft.align: left
#| fig-pos: "H"
...
gt(All_ROC_round) %>% 
  tab_footnote(footnote = "Note: AUC = Area Under the Curve. DP = Discrimination Power.") %>% 
    tab_options( footnotes.multiline = TRUE)
```

The Polsby-Popper method, while having the second highest AUC (i.e.
`r All_ROC_round$AUC[All_ROC_round$Feature == "Polsby–Popper [score/9]"]`),
was statistically different compared to the permuted valid score (*D* =
`r round(Results_ROCcomp[Results_ROCcomp$Feature == "pp_M",]$D,2)`, *p*
= \< .05). Hence, counter-intuitively, although the Polsby-Popper showed
descriptively larger AUC than the standardized perimeter, it did not
significantly differ from the permuted validity score, see @tbl-03. We
...
specificity (i.e. at a sensitivity of 80%, *D =*
`r round(pROC::roc.test(ROC_listed[[10]], ROC_listed[[11]],paired = TRUE, method="sensitivity", sensitivity=0.8)$statistic,2)`,
*p* = *n.s.*) nor sensitivity (i.e. at a specificity of 80%, *Z =*
`r round(pROC::roc.test(ROC_listed[[10]], ROC_listed[[11]], method="specificity", paired = TRUE,specificity=0.8)$statistic,2)`,
*p* = *n.s.*). We then compared the Polsby-Popper to the standardized
perimeter. We found higher specificity for standardized perimeter than
the Polsby-Popper method (at a sensitivity of 80 %, *D =*
`r round(pROC::roc.test(ROC_listed[[11]], ROC_listed[[4]], paired = TRUE, method="sensitivity", sensitivity=0.8)$statistic,2)`,
*p* =
`r round(pROC::roc.test(ROC_listed[[11]], ROC_listed[[4]], paired = TRUE,method="sensitivity", sensitivity=0.8)$p.value,2)`).
See also @fig-apx01 for the density plot's distribution across
...
```{=html}
<!--# The results also show that the permutations significantly
improved the AUC for topological validity (*D* =
...
`r cs("I don’t think you need any of the sections from here onwards for a pre-registration/registered report However, saying this, I think you can move some bits of the discussion section into the results section.I do not think you should discuss limitations right now", done = 1)`
...
In *phase II*, we will attempt at validating these criteria on a yet-to
be acquired dataset.
...
negatives @apx-false-positive-and-negative-analysis. But individual
space-forms can also be explored in @apx-all-participants-spatial-forms.
...
#| apa-note: 
#|    - Red = Classified as Synesthetes, Blue =  as controls. 
#|    - All method's score have been standardized (*z-score*). x axis trimmed between -3 and 3 z-scores. Vertical line indicate respectively 20, 50 (median) and 80 percentiles of each distributions.
...
#| apa-note: "Blue = Synaesthetes, Red = Controls. Each line represents a participants classification in SSS or control by each test at the given threshold. Blue highligh are Synesthetes who are systematically classified as control by all 5 tests. Red highlight are controls who are systematically classified as synesthetes by all 5 tests."
#| fig-position: H
...
# False positive and negative analysis {#apx-false-positive-and-negative-analysis}
...
# All participants spatial forms {#apx-all-participants-spatial-forms}
