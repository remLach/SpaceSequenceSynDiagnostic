...
segments and polygons. These novel features are designed to take
advantage of the sequentiality or ordinality between the responses and
the geometrical properties at the spatial-form level, see
...
\caption{\label{fig-Ex01}Deisgn for novel method for consistency test:
from between trial varibility to Segment and polygons formed by the
space-forms using geography tools.}

\centering{

\pandocbounded{\includegraphics[keepaspectratio]{RPlot02.png}}
...
(see also \href{https://osf.io/nu5v4/overview}{https://osf.io/nu5v4})
and an additional dataset was gently provided by private communications
with Professor Ward. To match the other datasets, stimuli of the
weekdays and months categories were translated from Dutch to English in
the dataset from (\citeproc{ref-vanpetersen2020}{Van Petersen et al.,
2020}). For the number category we included only digits from 0 to 9 that
where presented across all datasets (i.e.~excluding the stimuli ``50''
and ``100'' for numbers in Van Petersen et al.
...
\pandocbounded{\includegraphics[keepaspectratio]{2.RegisteredReport_MS2_files/figure-pdf/fig-myplot01-1.pdf}}
...
\citeproc{ref-ward2022a}{Ward, 2022}).

\paragraph{Area and perimeter between
repetitions.}\label{area-and-perimeter-between-repetitions}

Formally, the area (see Equation~\ref{eq-area}) and perimeter (see
Equation~\ref{eq-perim}) are calculated from the triangle (i.e.~given
three repetitions) formed by the \emph{x} and \emph{y} coordinates of
the three repetitions of each stimuli (i.e.~\emph{(x1, y1), (x2, y2),
(x3, y3)}).
...
similarly as originally described in Galton
...
categories in non-chronological order The form-based features computed
...
features. The rationale is that genuine synaesthetes would have
consistent topological features of spatial-forms repetitions regardless
of presentation order, whereas controls would show random variation. The
...
\pandocbounded{\includegraphics[keepaspectratio]{2.RegisteredReport_MS2_files/figure-pdf/fig-myplot00-1.png}}
...
features}{\label{fig-myplot2}}}}

\pandocbounded{\includegraphics[keepaspectratio]{2.RegisteredReport_MS2_files/figure-pdf/fig-myplot2-1.pdf}}
...
{\caption{{ROC analyses of all features}{\label{tbl-mytb03}}}
\vspace{-20pt}}
...
{\caption{{Descriptives of each features}{\label{tbl-mytb04}}}
\vspace{-20pt}}
...
datasets (See Figure~\ref{fig-apx08}). These differences could be
mitigated by using individually standardized coordinates M = 0.05
(\emph{SD = 0.06}) Rothen et al. (\citeproc{ref-rothen2016}{2016}), M =
0.08 (\emph{SD = 0.15}) Ward (\citeproc{ref-ward2022a}{2022}), M = 0.09
(\emph{SD = 0.15}), Ward2, see also Table~\ref{tbl-mytb04}.
...
the non permuted one (D = 3.12, \emph{p} \textless{} .05). Both criteria
display bimodal distributions across the groups (see
Figure~\ref{fig-apx01}).
...
1.53 valid of 9 forms) and the other for standardized perimeter between
the repetitions of each stimuli (AUC = 78.8; DP = 2.06, cut-off = 1.73
z-scores). However, at the proposed cut-off, the permuted validity leads
to higher specificity than perimeter (78.55 vs. 75.78) indicating better
...
negatives). See Figure~\ref{fig-apx01} for the densities of each
standardized features suggesting bi-modal distribution.
...
different features best fit different datasets, see
Figure~\ref{fig-myplotapx02}. Interestingly the best features by
datasets differ between the data sources. While the standardized area
indeed lead to the best AUC for the data from Rothen et al.
(\citeproc{ref-rothen2016}{2016}), the perimeter leads to better AUC in
Ward et al. (\citeproc{ref-ward2018}{2018}). The permuted validity leads
to AUC \textgreater{} 80 across all datasets.
...
questionnaire scores (see \citeproc{ref-ward2018}{Ward et al., 2018}),
taking the 10-0\% highest questionnaire scores against the 10 \% worst
(i.e.~90-100 \%) . We further sub-sampled by 10 \% questionnaire scores
and recalculated AUC, sensitivity and sensibility, see
Figure~\ref{fig-myplotapx03} for AUC, DP, sensitivity and specificity.
Using this method we can see that the total AUC remains relatively
stable across percentiles.
...
definition differ in consistency (\citeproc{ref-root2025}{Root et al.,
2025}). Hence, we made a correlation matrix with the questionnaire
scores and all the features, see Appendix. The two highest correlations
are indeed between the questionnaire score and perimeter (r = .58) and
permuted validity (r = .50), see Figure~\ref{fig-myplotapx04}.

Some of the participants might have been originally mis-attributed to
control or SSS, see Figure~\ref{fig-myplotapx07}. Descriptively, we also
found about \textasciitilde6 \% of the full sample's synesthetes to be
consistently classified as controls (i.e.~consistent false positive) by
the five methods with the highest AUC Figure~\ref{fig-myplotapx08}.
\textasciitilde3 \% controls are consistently classified as SSS
(i.e.~consistent false negative) Figure~\ref{fig-myplotapx09}.
...
between the forms. The figure in the appendix
...
(\citeproc{ref-sachdeva2024}{2024}), Stage 1 in-principle acceptance).
Hence for the stimulus levels feature we will compare the area and
...
in-principle acceptance) (\url{https://osf.io/6wn4m}).
...
on the dataset (see Figure~\ref{fig-myplotapx02}) it is likely that the
best features here will not match with the best feature in the to be
collected dataset.
...
space-form level. Hence, the area test from Rothen et al.
(\citeproc{ref-rothen2016}{2016}) led to worse classification than the
perimeter test with a larger sample. Furthermore we could prove similar
performances using a different approach based on the space-form level
(i.e.~topological validity).

Further analyses indicates variable optimal tests in terms of AUC
depending on the datasets (see Figure~\ref{fig-myplotapx02}).
Considering only the data by Professor Ward that contains questionnaire
data, we comparable patterns of AUC and DP for more extreme Synaesthete
and Control groups when sub-sampled by questionnaire's score
distribution percentiles (see Figure~\ref{fig-myplotapx03}).
Nevertheless the standardized perimeter seems to lead to better AUC and
DP in more extreme scores. Also, the standardized perimeter lead to
slightly higher correlation with the questionnaire scores (r = .57) than
permuted topological validity (r = .51), see
...
(i.e.~retinotopy, sonotopy or somatotopy
\citeproc{ref-eagleman2009}{Eagleman \& Goodale, 2009}). Therefore
...
\section{Feature distributions}\label{apx-featDensity}

Regarding the distributions of each features across the SSS vs.
controls, Figure~\ref{fig-apx01} we compared the density plots of each
...
\pandocbounded{\includegraphics[keepaspectratio]{2.RegisteredReport_MS2_files/figure-pdf/fig-apx01-1.pdf}}
...
that some of the tests led to 100 \% Specificity in Van Petersen
dataset.
...
\pandocbounded{\includegraphics[keepaspectratio]{2.RegisteredReport_MS2_files/figure-pdf/fig-myplotapx02-1.pdf}}
...
{\caption{{Density plots of all the features comparing SSS and controls
by dataset}{\label{fig-apx08}}}}

\pandocbounded{\includegraphics[keepaspectratio]{2.RegisteredReport_MS2_files/figure-pdf/fig-apx08-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{2.RegisteredReport_MS2_files/figure-pdf/fig-myplotapx03-1.pdf}}
...
questionnaire score.\\
Works only with Ward's aggregated data, see
Figure~\ref{fig-myplotapx04}.
...
\pandocbounded{\includegraphics[keepaspectratio]{2.RegisteredReport_MS2_files/figure-pdf/fig-myplotapx04-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{2.RegisteredReport_MS2_files/figure-pdf/fig-myplotapx06-1.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{2.RegisteredReport_MS2_files/figure-pdf/fig-myplotapx07-1.pdf}}
...
\emph{fail} all five tests}

\centering{

\pandocbounded{\includegraphics[keepaspectratio]{2.RegisteredReport_MS2_files/figure-pdf/fig-myplotapx08-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{2.RegisteredReport_MS2_files/figure-pdf/fig-myplotapx09-1.pdf}}
