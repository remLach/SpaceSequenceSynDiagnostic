...
New here: ds_Rothen's includes the group as a corresponding of SynQuest
in Ward.
...
-   For each features (replicated + new):
...
In sum, I want 2 data sets: ds: item level ds_Q: ID level
...
ds_Q_Rothen <- merge(ds_Q_Rothen, ds_Rothen, by = c("ID","dataSource"), all = TRUE)

ds_Q_Rothen <- ds_Q_Rothen %>%
  select(ID, group, dataSource)
...
ds %>%
    ...
    geom_text(aes(x = x_zs+0.1, y = y_zs+0.1), colour = "black", size = 3) +
    geom_path(aes(x = x_zs, y = y_zs, group = repetition), alpha = 0.2) +
    geom_text(aes(x = x_zs+0.1, y = y_zs+0.1), size = 0.5, alpha = 0.5) +
    geom_text(aes(x = -2, y = 0.25, label = "Consistency Area:")) +
    geom_text(aes(x = -2, y = 0, label = round(Consistency,5))) +
    facet_grid(~Cond) +
    theme_minimal()
```
...
```{r}
# ROC_Cons <- roc(group ~ Consistency, ds_Q, 
#                 percent=TRUE,
#                 # arguments for ci
...
# 
# All_ROC[1,] <- c("Consistency", round(ROC_Cons$auc,4), best_coords,as.numeric(ci_auc[1]),as.numeric(ci_auc[2]))
...
ds %>%
    ...
    geom_text(aes(x = -2, y = 0.25, label = "Consistency Area:")) +
    geom_text(aes(x = -2, y = 0.15, label = round(Consistency,5))) +
   
    geom_text(aes(x = -2, y = -0.25, label = "SD:")) +
    geom_text(aes(x = -2, y = -0.35, label = round(SD_ID_x,3))) +
    geom_text(aes(x = -1.5, y = -0.35, label = round(SD_ID_y,3))) +
...
The question is, should I sum the features across Conditions or average them? 
I know some conditions contain responses at the exact same coordinates. 
Also the conditions don't have the same number of stimuli, i.e.:
- weeks: 7 
- months: 12
- numbers: 10
Hence months are more likely to have self-intersections than weeks. Is that important?
...
## 4.2. ROC
```{r}
ROC_LineInter <- roc(group ~ Sum_lineInter , ds_Q, 
                percent=TRUE,
                # arguments for ci
                ci=TRUE, boot.n=100, ci.alpha=0.9, stratified=FALSE,
                # arguments for plot
...
ds_Q <- merge(ds_Q, ds_segm %>% ungroup() %>% group_by(ID) %>% filter(row_number() == 1) %>% select(ID, GA_segm_leng),by = "ID")
```
...
## 9.1. Example
```{r}
IDList_simple   <- ds_poly$ID[ds_poly$isSimple]
IDList_NOTsimple <- ds_poly$ID[ds_poly$isSimple]

#IDEx_simple = sample(IDList_simple,1)
IDEx_simple <- "36540"
# IDEx_NOTsimple = sample(IDList_NOTsimple,1)
IDEx_NOTsimple <- "29213"
ds_poly %>% 
    filter(ID == IDEx_simple) %>%
    select(isSimple) %>%
    plot()

ds_poly %>% 
    filter(ID == IDEx_NOTsimple) %>%
    select(isSimple) %>%
    plot()
sf.colors(alpha = 0.2)
...
IDList_Valid    <- ds_Q$ID[ds_Q$Sum_isValid == min(ds_Q$Sum_isValid)]
IDList_NOTvalid <- ds_Q$ID[ds_Q$Sum_isValid == max(ds_Q$Sum_isValid)]
...
## 10.2. ROC
```{r}
ROC_isValid <- roc(group ~ Sum_isValid , ds_Q, 
                percent=TRUE,
                # arguments for ci
                ci=TRUE, boot.n=100, ci.alpha=0.9, stratified=FALSE,
                # arguments for plot
...
# Best threshold using Youden's J
best_coords <- coords(ROC_isValid, "best", ret = c("threshold", "sensitivity", "specificity","ppv","npv"), best.method = "youden")
knitr::kable(best_coords)

ci_auc  <- ci.auc(ROC_isValid)

All_ROC[10,] <- c("isValidPoly", round(ROC_isValid$auc,4), best_coords,as.numeric(ci_auc[1]),as.numeric(ci_auc[2]))
```
...
All_ROC[11,] <- c("isClockwise", round(ROC_isClosckwise$auc,4), best_coords,as.numeric(ci_auc[1]),as.numeric(ci_auc[2]))
```
...
hist(ds$segment_angle_deg,360)
hist(ds$turning_angle_deg,360)
...
```{r}
roc.test(group ~ Sum_isValid + Consistency_zs , ds_Q, partial.auc = c(100, 90), percent = TRUE,
         ci=TRUE, boot.n=100, ci.alpha=0.9, stratified=FALSE,
         # arguments for plot
...
ds_ML <- ds_Q %>% 
  select(group,Consistency,SD_ID_xsc,SD_ID_ysc,Sum_lineInter, 
         GA_lineInter,GA_BtwInter,GA_segm_leng,GA_BtwDist,GA_areaPoly,
         GA_isSimple,Sum_isValid,Sum_isClockwise,GA_area_convhull) %>%
  na.omit() %>% 
