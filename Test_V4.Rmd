...
ds$SynQuest_NR <-  ds$ID %in% ID_Q_NR$ID


ggplot(ds,   aes(x = GA_lineInter, y = SynQuest_NR, group = SynQuest_NR, fill = factor(stat(quantile)))) +
  stat_density_ridges(
    geom = "density_ridges_gradient", calc_ecdf = TRUE,
    quantiles = 4, quantile_lines = TRUE
  ) +
  scale_fill_ordinal(name = "Quartiles")


ds %>%
  group_by(SynQuest_NR) %>%
  summarize(quant10 = quantile(GA_lineInter, probs = 0.1), 
            quant20 = quantile(GA_lineInter, probs = 0.2),
            quant30 = quantile(GA_lineInter, probs = 0.3),
            quant40 = quantile(GA_lineInter, probs = 0.4),
            quant50 = quantile(GA_lineInter, probs = 0.5),
            quant60 = quantile(GA_lineInter, probs = 0.6),
            quant70 = quantile(GA_lineInter, probs = 0.7),
            quant80 = quantile(GA_lineInter, probs = 0.8),
            quant90 = quantile(GA_lineInter, probs = 0.9))

ggplot(ds,   aes(x = GA_lineInter, group = SynQuest_NR,fill =SynQuest_NR )) +
    stat_ecdf(geom = "step") +
    geom_vline(xintercept = 1, alpha = 0.2, colour = "red")
```
