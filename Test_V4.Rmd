...
ggplot(ds,   aes(x = GA_lineInter, group = SynQuest_NR,fill =SynQuest_NR, colour = SynQuest_NR)) +
  ...
  geom_segment(data = ecdf_vals,
               aes(x = x_intersect, xend = x_intersect,
                   y = 0, yend = y_intersect,
                   color = SynQuest_NR),
               linetype = "dashed") +
  geom_segment(data = ecdf_vals,
               aes(x = x_min, xend = x_intersect,
                   y = y_intersect, yend = y_intersect,
                   color = SynQuest_NR),
               linetype = "dotted") +
  geom_point(data = ecdf_vals, aes(x = x_intersect, y = y_intersect)) +
  geom_text(data = ecdf_vals,
            aes(x = x_intersect, y = y_intersect,
                label = paste0("(", round(x_intersect, 2), ", ", round(y_intersect, 2), ")")),
            vjust = -1.2, hjust = 0.5) +
    labs(x = "Avererage number of intersections", y = "Percentiles", group = "Legend") 
...
roc_obj <- roc( ds$SynQuest_NR,ds$GA_lineInter)
