...
# Add to ds_Q
ds_Q <- merge(ds_Q, ds %>% ungroup() %>% group_by(ID) %>% filter(row_number() == 1) %>% select(ID, GA_lineInter),by = "ID")
...
2.1.2. Self-intersections across repetitions dirty

Simple dirty: count ignoring repetitions (so the last of each repetition should be then counted with the next)

```{r}
# ds <- ds %>%
#   group_by(ID, Cond) %>%
#   mutate(nSegments_wrep = length(stimulus)-1)
# 
# 
# # Number of intersections for each ID X Cond X repetition
# ds <- ds %>% 
...
#   ungroup() %>%
#   group_by(ID, Cond) %>%
#   mutate(nLineCross_wRep = (count_self_intersections(x,y, verbose = FALSE)))
# 
# # Linear transformation: chances for each segment to intersect:
# ds <- ds %>% 
#   group_by(ID, Cond) %>%
#   mutate(mean_lineInter_wRep = nLineCross_wRep/nSegments)
# 
# # Average per ID
# ds <- ds %>%
#   group_by(ID) %>%
#   mutate(GA_lineInter_wRep  = mean(mean_lineInter_wRep ))
# 
# 
# roc_line <- roc(SynQuest ~ mean_lineInter_wRep, ds, 
#                 percent=TRUE,
#                 # arguments for ci
...
1 0.09279577    84.14634    54.72527

To:
 threshold sensitivity specificity
1   3.89899    74.60943    60.52197

We lose sensitivity but gain specificity, seems the good direction!

## 2.2.  Self-intersections across repetitions (with sf)
...
## 2.2.1. Between repetitions: 
```{r}

# 
# InterBtwRep <- function(segm1,segm2,segm3) {
# 
...
# 
#   return(nInter = nrow(st_coordinates(i12)) + nrow(st_coordinates(i13)) + nrow(st_coordinates(i23)))
# 
#   if (Vis) {
...
# }
# 
# InterBtwRep(ds_segm[1,]$geometry,ds_segm[2,]$geometry,ds_segm[3,]$geometry)
# 
# ds_segm %>% 
#   group_by(ID, Cond, Group) %>%
#     summarise(across(everything(), InterBtwRep), .groups = "drop")
# 
# ds_segm %>% 
#   group_by(ID, Cond, Group) %>%
#   summarise(geometry = {
#   InterBtwRep(geometry[[1]],gometry[[2]],geometry[[3]])
#   }, 
#   .groups = "drop"
#   )
# 
...
#   filter(ID == "1005_SeMi") %>%
#   filter(Cond == "month") %>%
#   group_modify(~ InterBtwRep(geometry))
...
#   group_by(ID, Cond, repetition) %>%
#   mutate(geometry = st_area(st_concave_hull(geometry, ratio = 0.5)))
# 
# 
...
prop.table(table(SynCons,SynQuest,SynGroup)) * 100
```
...
```{r}
M1 <- glm(SynGroup ~ GA_lineInter , family=binomial, data = ds)
M2 <- glm(SynGroup ~ Consistency , family=binomial, data = ds)
anova(M1,M2)


M1.1 <- glm(SynQuest ~ GA_lineInter , family=binomial, data = ds)
M2.1 <- glm(SynQuest ~ Consistency , family=binomial, data = ds)
anova(M1.1,M2.1)
