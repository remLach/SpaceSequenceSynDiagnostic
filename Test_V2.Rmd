...
# 4.1. Line crossings
Let's say 0.2 as threshold. This means that each segment crosses on average 0.2 times.

```{r}
ds$SynLine <- ds$totLineCross < 0.2
```
...
Maybe could automathize by pulling the inconsistent but with few line crossings. 

```{r}
InconsbutLine_ID <- unique(ds$ID[!ds$SynCons & ds$SynLine])
```

### Inconsitent ID's that pass line criteria:
- 29489:
Look at this, I would say this is indeed synesthesia:
```{r}
ds %>%
    filter(ID %in% 29489) %>%
    group_by(stimulus) %>%
...
- 32074 has a lot of ine crossings!
- 40867 passes altough higlhy inconsistent
- 44296
- 46320
- 48114 
- 49369
...
```{r}
ds %>%
  filter(ID %in% InconsbutLine_ID[1:5]) %>%
  group_by(stimulus) %>%
  arrange(stimulus) %>%
  arrange(ordered(stimulus, levels = c("Monday", "Tuesday", "Wednesday", "Thursday", "Friday","Saturday","Sunday"))) %>% arrange(ordered(stimulus, levels = c("January", "February", "March", "April", "May","June","July","August","September","October","November","December"))) %>%
  ggplot(aes(x = x_zs, y = y_zs, group = stimulus, label = stimulus, fill = stimulus)) +
