...
## Heterogeneity and homogeneity of SSS {#sec-heterogeneity-and-homogeneity-of-sss}

SSS spatial (visuo-spatial) forms are idiosyncratic, which results in
...
some SSS experiences involve three-dimensional (3D) and two-dimensional
(2D) spatial arrangement [@eagleman2009a; @price2013]. Another source is
the reference frame, for example some SSS, the spatial forms appear to
occupy an external space around the body (*i.e.* projector) while for
others the forms are experience in an internal space (*i.e.* associator)
[@dixon2004; @smilek2007]. Further variability can be explained by
temporal-spatial properties, some SSS reporting they can manipulate
their spatial forms such as "zooming" in and out, rotating or shifting
perspectives [@gould2014]. Lastly, the shape, complexity and layout of
the spatial forms are also heterogeneous for example ovals, lines,
loops. Although some forms are more recurrent, such as ovals for months
[@eagleman2009a].

Despite the after mentioned heterogeneities, SSS has some
phenomenological charachteristics. *Automaticity*: the *inducer*
...
inducer. *Developmentally early*: the experience was already present
during childhood. *Consciousness*: The concurrent is consciously
perceived. *Consistency*: the inducer-concurrent pair remains stable in
time. These criteria can be tested both with self reported questionnaire
(i.e. for development and consciousness), the other can be assessed more
experimentally.

## Consistency tests {#sec-consistency-tests}

Beside subjective self-report questionnaire, more objective tests have
been developed to test for consistency [@baron-cohen1993]. In practice,
those test are mostly used to validate self-reports for experimental
needs (i.e. identifying SSS and controls).

Brang et al.[-@brang2010], evaluated consistent responses when the
distance between same stimuli (i.e. January and January) was within 1.96
z-scores compared to the adjacent stimuli (i.e. February). This criteria
might however be to conservatory since it detected synesthesia in 4 of
...
accuracy [@rothen2013a]. The rationale being that synesthetes should
give be consistent in responding (i.e. less variable) across time
delayed repetitions. A measure of consistency between repetitions has
been the total area drawn by the coordinates for certain stimuli across
repetitions [@rothen2016], sometimes combined with standard deviation of
responses [@ward] or permuting the responses to compare single responses
to a chance level calculated from permutationsas in Root [-@root2021].
Consistency for SSS has followed a similar design: participants have to
position the potential inducer on a computer screen . A measure of
consistency between the x and y coordinates is then compared. The total
area between the responses of same inducer has been suggested to be used
[@rothen2016]. [@ward2018]
...
[@rothen2016;@vanpetersen2020a] in laboratory. Each stimuli is presented
randomly and sequentially centrally on the screen. Participant are
instructed to click on the screen position where they visualize them.
...
Additional data was gently provided by private communications with Pr.
...
`r All_n - All_n2` empty responses including responses with the same
coordinates across conditions and repetitions, causing the depletion of
`r N1-N2` participants. `r N2-N3` participants were excluded for having
less than 4 coordinate points since this would impeach computing
polygons, see @sec-category-level-additional-features. Note also that
this means `r sum(tmp2$totCond < 3)` participants did not have
coordinates in all conditions, for example no coordinates for numbers.
Then, x and y coordinates were then separately normalized (z-score) per
participant.
...
## *Phase I.* Population {#sec-phase-i.-population}
...
Conceptually more consistent responses to the same stimuli should be
closer. This distance can be computed as the area or the perimeter
formed by the x,y coordinate between the repetitions (i.e. (x1, y1),
(x2, y2), (x3, y3)). With three repetitions, the area is calculated as
in using the formula @eq-area and the perimeter @eq-perim.
...
Since the screen sizes differ, we computed the area on individually
z-score transformed x,y coordinates. The area of each stimuli is then
averaged for each participants.
...
cut-off = 0.17) and then the average normalized perimiter between the
...
sel_dsQ <- ds_Q %>%
  select(c(QuestScoreRL,feature_list)) %>%
