...
pre-registered report, available data from 666 participants is used to
...
<!--#  A stringent definition or charachterization of Synesthesia stipulates five assumptions [@deroy2013]. Consciousness: The concurrent is consciously perceived, i.e. a SSS would say that 9 is in a specific position. Automaticity: the inducer automatically triggers the concurrent, i.e. 9 is automatically perceived in a specific spatial position. Unidirectionality: while the inducer triggers the concurrent, the concurrent does not trigger the inducer, i.e. while 9 is perceived in a specific spatial position, this position is not perceived as number 9 (but see [@kadosh2005; @johnson2007] for bidirectinal synesthesia). Developmentally early: synesthetic experiences were already present during childhood. Consistency: the concurrent's reported by the same inducer are stable in time, i.e. the specific position triggered by 9 remains the same if asked at a different moment . -->
...
The optimal criterion idneeds to be informed about the order between
