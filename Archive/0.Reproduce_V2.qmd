...
ds_Rothen$X[ds_Rothen$X == -1] <- 0
ds_Rothen$Y[ds_Rothen$Y == -1] <- 0
...
Data found here: <https://reshare.ukdataservice.ac.uk/852530/> The ID's have been renamed across datasets -.-'. So to check for data matches, I sort the dataset.
...
# To_Check %>%
#   filter(Diff == max(To_Check$Diff)) # Subject 4!
# 
# ds_Rothen %>%  
#   filter(subject %in% 4)
```
