...
-   Rothen [@rothen2016].
Data can be found here: <https://osf.io/6hq94/files/osfstorage> and here:
<https://reshare.ukdataservice.ac.uk/852530/>
...
Data found here:
<https://reshare.ukdataservice.ac.uk/852530/>
The ID's have been renamed across datasets -.-'. So to check for data matches, I sort the dataset.
...
max(sort(ds_rothen_ID$triangle_area_GA)-sort(ds_Rothen_aggregated$`area`))
...
# Uff, we are getting there:

tmp1 <- ds_rothen_ID %>%
  arrange(triangle_area_GA) %>%
  select(ID, group, triangle_area_GA)
...
# Realised now that the ID match: I just need to keeb the numbers in ds_rothen_ID$ID 
ds_rothen_ID$subject <- as.numeric(gsub(".*?([0-9]+).*", "\\1", ds_rothen_ID$ID))
# Got it!!!
sum(!ds_rothen_ID$subject %in% ds_rothen_ID$subject )
```
