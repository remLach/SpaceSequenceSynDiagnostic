...
author:
  - name: Rémy Lachelin
    ...
    email: remy.lachlein@fernuni.ch
    affiliations:
...
sets <- (list(Prev = ds_Q$ID, New = ds_Ward2_Q$ID))

library(ggvenn)
ggvenn(sets, fill_color = c("#E41A1C", "#377EB8"))

# Merge is fucking up 
ds   <- merge(ds,ds_Ward2,all.x = TRUE)
# tmp %>% group_by(dataSource, ID, Cond, repetition) %>% summarize(ntrial = length(x)) %>% filter(ntrial <= 3)
ds_Q <- merge(ds_Q,ds_Ward2_Q,all.x = TRUE)

ID_Ward2 <- ds_Ward2_Q$ID

rm(ds_Ward2_Q,ds_Ward2)
```
...
package to compute segment based features. We compute polygon based
features. These methods are on the form or category level. The rationale
here is to see whether when considering the stimuli as ordered
coordinates, i.e. as segments or polygon, they are consistent *between*
repetitions.
...
## Compactness ratio: Area to Perimeter

See

<https://jblindsay.github.io/ghrg/Whitebox/Help/CompactnessRatio.html>

```{r}
ds_Q$repAreatoPerim <- ds_Q$triangle_perim_GA_zs/ds_Q$rep_area_GAzs 
...
For each coordinates we can also compute the spread, i.e. how much in
average each coordinate is distant to a centroid (i.e. mean of the
coordinates) that is calculated between conditions but including all
repetitions.

Formal definition: \$ \frac{\sum \lvert \text{centroid}_x - x \rvert}
{\text{length}(x)} \$
...
ds <- ds %>%
  group_by(ID,Cond) %>%
  mutate(x_MidID = mean(x_zs, na.rm = TRUE), y_MidID = mean(y_zs, na.rm = TRUE))

#     For each measured color point, calculate its distance from the ‘middle’.
ds <- ds %>%
  group_by(ID,Cond) %>%
  rowwise() %>%
  mutate(x_MidDist = abs(x_MidID - x_zs),y_MidDist =  abs(y_MidID - y_zs ))
#     Add up all these distances and divide by the total number of color points.
ds <- ds %>%
  ungroup() %>%
  group_by(ID, Cond) %>%
...
ds_Q <- right_join(ds_Q, 
                   (ds %>% 
                      ...
                      group_by(ID) %>% 
                      # filter(row_number() == 1) %>%
                      summarize(M_spread_y = mean(spread_y)) %>%
                      select(ID, M_spread_y)), by = "ID")
...
ds_Q <- right_join(ds_Q, 
                   (ds %>% 
                      ...
                      group_by(ID) %>% 
                      # filter(row_number() == 1) %>%
                      summarize(M_spread_x = mean(spread_x)) %>%
                      select(ID, M_spread_x)), by = "ID")
...
```{r}
ds_poly$areatoPerim <- ds_poly$area/ds_poly$perimeter

ds_poly <- ds_poly %>%
  group_by(ID) %>%
  mutate(areatoPerim_GA = mean(areatoPerim, na.rm = TRUE))

ds_Q <- right_join(ds_Q, 
                   ds_poly %>% st_drop_geometry() %>% ungroup() %>% group_by(ID) %>% filter(row_number() == 1) %>% select(ID, areatoPerim_GA),
                   by = "ID")
...
# *Phase I.* Results - permute repetitions
...
for(Perm_n in 1:n_perms){
  ...
  for(ID_n in 1:length(unique(ds$ID))){
    ...
    ds_perm[ID_n,Perm_n] <- correl_perm$GA_corr
    k <- k + 1
    setTxtProgressBar(pb, k)
  }
}
...
# Loop by features:
for(fit_n in 1:length(feature_list)){
  ROC_here <- Comp_ROC(ds_Q2, "group", feature_list[fit_n],"ID",feature_direction[fit_n])
...
# quant <- c(0,quant)
# quant <- c(quant, max(ds_Q2$`questionnaire score`))

# AUC_perctils <- data.frame(matrix(nrow = (length(feature_list)*(length(quant)/2)), ncol = 3))
# # data.frame(feat = character(),
# #                       AUC  = integer(),
# #                       perct = integer(),
# #                        nrow() =length(feature_list))
# AUC_perctils$feat <- rep(NaN,3)
...
for(quant_n in 1:(length(quant)/2)){
  ...
  # Loop by features:
  for(fit_n in 1:length(feature_list)){
    ROC_here <- Comp_ROC(ds_Q2_quant_n, "group", feature_list[fit_n],"ID",feature_direction[fit_n])
