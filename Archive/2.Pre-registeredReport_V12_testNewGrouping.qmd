...
\<.203 % of the total screen area to classify as SSS was suggested i.e.
...
```{r}
# ds_Q$group[ds_Q$`questionnaire score` <= 19] <- "Syn"
# ds_Q$group[ds_Q$`questionnaire score` > 19]  <- "Ctl"
# 
# ds$group[ds$ID %in% ds_Q$ID[ds_Q$group == "Syn"]] <- "Syn"
# ds$group[ds$ID %in% ds_Q$ID[ds_Q$group == "Ctl"]] <- "Ctl"


# ds_Q$group[ds_Q$dataSource %in% c("Ward","Ward2")] <- "Ctl"
# ds_Q$group[ds_Q$`Q2_days  (1=present, 0=absent)` | ds_Q$`Q2_numbers (1=present, 0=absent)` | ds_Q$`Q2_months  (1=present, 0=absent)`] <- "Syn"
# 
# ds_Q$group <- as.factor(ds_Q$group)
# 
# ds$group[ds$ID %in% ds_Q$ID[ds_Q$group == "Syn"]] <- "Syn"
...
ds_Q <- ds_Q %>% filter(dataSource != "Ward2")
ds   <- ds %>% filter(dataSource != "Ward2")
```
...
in @tbl-mytb1 the original sample's reported descriptives.
...
Regarding the syntheses, we can only describe their profile from the
data by Pr. Ward (i.e. from
`r sum(ds_Q$dataSource %in% c("Ward","Ward2"))` cases. These profiles
are described only for the stimulus categories used in the consistency
test (i.e. number, weekdays and month). From the venn-diagram of
@fig-myplot1 we see that only 18% of this sample does neither
self-report having SSS, nor space-forms for Numbers, Days or Month.
...
ID_all    <- na.rm(ds_Q$ID[ds_Q$dataSource %in% c("Ward","Ward2")])
ID_group  <- na.rm(ds_Q$ID[ds_Q$group =="Syn" & ds_Q$dataSource %in% c("Ward","Ward2")])
...
x <- list(All = ID_all, Group = ID_group, Numbers = ID_number, Day =  ID_days, Month =ID_month)
...
ggVennDiagram(x, set_color = c("brown","brown","black","black","black")) +
...
with Receiver Operator Characteristics (ROC) analyses. Area Under the
Curve (AUC) is used to determine which feature is best at classifying
SSS from Controls. The optimal cut-off's are calculated with Youden's
index [@youden1950]. Discriminant Power (DP) is used as an additional
estimate of discrimination performance according to the optimal cut-off
...
Additional analyses in the appendix try to address several concerns
raised in the literature of consistencyt test: that some criteria might
be more beneficial towards a certain type of SSS than others and
circularity. For the concern that some criteria might bias towards types
of synesthesia, we compared the groups by sub-type of SSS (i.e.
weekdays, months and numbers) with the classifications using the
cut-offs for permuted validity and perimeter. In an attempt to
circumvent the circularity of perfectioning consistency test based ons
elf-reported groups, we attempted two additional approaches: first
testing sub-samples depending on percentiles of the questionnaire score
sample distribution @sec-sm1. The rationale is that a good consistency
test should have similar discriminative performances for strong SSS and
weaker SSS. To do this we re-sampled the SSS and controls based on their
questionnaire distribution by percentiles, i.e. comparing 10% highest
questionnaire 10% worst, then 20% vs 20%, ect. Second correlating the
questionnaire scores with the results from different features extracted
from the consistency test @sec-sm2-correlation-with-self-report.
Finally, since we will attempt to predict the best diagnostic features
of a to be collected dataset, we also carried analyses across the
different datasets separately, to test wheter each consistency test's
feature remain constant, see @sec-SM3byds .
...
We compute the area in term of % of the screen area to be able to
...
Each participant's form is tested for validity as a boolean for each
repetitions and averaged, averaged over the 9 forms per participants
(i.e. 3 categories X 3 repetitions). Hence it can span from 1 (all 9 are
valid) to 0 (none of the 9 forms are valid).
...
### Form level: additional improvements {#sec-category-level-additional-improvements}
...
Descriptively, the area between repetition for SSS was larger than
previously literature: 0.26% , compared to 0.14% in [-@rothen2016] and
0.15 % in [@ward2018]). This difference seems to be mainly driven by the
dataset from [@ward] with respectively 0.26% area. Descriptively, this
might be explained by a more variable sample \[*SD = 0.50* % in [-@ward]
compared to *SD = 0.09 %* in @rothen2016\] see @tbl-mytb4. However these
are more similar across datasets using standardized coordinates (M =
0.05 (*SD = 0.06*) [@rothen2016], M = 0.08 (*SD = 0.15*) [@ward], M =
0.09 (*SD = 0.15*)). Hence, for the perimeter, we only consider
z-scores. All other feature's averages and standard deviations are
described in @tbl-mytb01.
...
```{r}
ds_Q$isValid_perm_M_inv <- 1 -  ds_Q$isValid_perm_M
AUC_test <- roc.test(pROC::roc(group ~ isValid_perm_M, ds_Q, direction= "<"),pROC::roc(group ~ Perimeter_zs, ds_Q, direction= ">"))

AUC_test_Venk <-roc.test(pROC::roc(group ~ isValid_perm_M_inv, ds_Q, direction= ">"),pROC::roc(group ~ Perimeter_zs, ds_Q, direction= ">"),method=c("venkatraman"))

ds_Q <- ds_Q %>% select( - isValid_perm_M_inv) # Remove this column otherwise will mess with the rest of the code
```

The ROC analysis suggest that the two best AUC are obtained from the
averaged permuted validity score (AUC =
`r round(All_ROC[All_ROC$Feature == "isValid_perm_M",]$AUC,2)`; CI =
\[`r round(All_ROC[All_ROC$Feature == "isValid_perm_M",]$ci_low,2)`-`r round(All_ROC[All_ROC$Feature == "isValid_perm_M",]$ci_high,2)`\],
DP = `r round(All_ROC[All_ROC$Feature == "isValid_perm_M",]$DP,2)`,
cut-off =
`r round(All_ROC[All_ROC$Feature == "isValid_perm_M",]$threshold,2)` or
`r round(All_ROC[All_ROC$Feature == "isValid_perm_M",]$threshold,2)*9`/9
valid forms) and the standardized perimeter between the repetitions of
each stimuli (AUC =
`r round(All_ROC[All_ROC$Feature == "Perimeter_zs",]$AUC,2)`; CI =
\[`r round(All_ROC[All_ROC$Feature == "Perimeter_zs",]$ci_low,2)`-`r round(All_ROC[All_ROC$Feature == "Perimeter_zs",]$ci_high,2)`\],
DP = `r round(All_ROC[All_ROC$Feature == "Perimeter_zs",]$DP,2)`,
cut-off =
`r round(All_ROC[All_ROC$Feature == "Perimeter_zs",]$threshold,2)`
z-scores). While both AUC do not differ between these (DeLong test: D =
`r round(AUC_test$statistic,2)`, *p* = `r round(AUC_test$p.value,2)`),
the total ROC do (Venkatraman test: `r AUC_test_Venk$statistic`, *p* =
`r round(AUC_test_Venk$p.value,3)`). However, at the proposed cut-off,
the permuted validity leads to higher specificity
(`r round(All_ROC[All_ROC$Feature == "isValid_perm_M",]$specificity,3)`
vs.
`r round(All_ROC[All_ROC$Feature == "Perimeter_zs",]$specificity,3)`)
...
hence perimeter is best at detecting SSS (i.e. less false positives).
See @tbl-mytb3 for all the features sorted by the largest AUC and
@fig-myplot2 for the descritive ROC curves of each features.
...
## Comparison of feature's distribution

Regarding the distribution's of each features across the SSS vs.
controls, @fig-myplot3 compares the density plots of each standardized
criteria (in order to make them comparable) which visually indicates a
bimodal distribution. Median of the z-scores per group are also
...
We descriptively compared whether the feature's criteria could bias
towards some SSS sub-types to the other. Using only the datasets from
Ward, we compared whether the detected SSS witht the two main feature's
(permuted validity and perimeter) applied cut-off led to detect more
self-reported SSS for specific space-forms for numbers, days or months.
This is illustrated by a Venn diagram showing each participant falling
in each of these five categories and their overlaps, see @fig-myplot4.
From this diagram we can assess that 30 % of the SSS do report space
forms for all the three categories and that are detected by both tests,
while 11 % with all categories are not detected by any of both features.
9 %

```{r}
#| label: fig-myplot4
#| fig-cap: "Avocado Venn Diagramm of self-report and tests"
#| apa-note: "Only data from Ward is included here. % from the toal of "

AllList       <- list(Group = ds_Q$ID[ds_Q$group == "Syn"]) # Important, this are all ID also thos without questionnaire
...
The extreme groups were defined by the percentiles on the questionnaire
sores, hence only the data from Ward is included there. We computed AUC,
sensitivity and sensibility for the 10-90 %, 20-80 %, 30-70 %, 40-60 %
sub-sampled participants depending on the distribution of the
synaesthesia questionnaire [see @ward2018]. The results for AUC
@fig-myplot5, sensitivity and specificity @fig-myplot6.

However we also found differences across the datasets, see @fig-myplot9
for AUC, @fig-myplot10 for sensitivity and specificity.
...
We will compute the same ROC analyses as in this pre-registration. For
