clear
dir("/Users/remylachelin/Documents/SpaceSequenceSynDiagnostic/SpaceSequenceSynDiagnostic/ReproduceRothen/analysis scripts") % RL 2025
fileName ='example_data.xls';   %enter current file name
a = readtable(fileName, "ReadRowNames", 1 )% xlsread; RL 2025 replace function
% [num, txt] = readtable(fileName);% xlsread; RL 2025 replace function
txt = a.Inducer % RL 2025
b = table2array(a(:,9)); % RL 2025 table2array
c = table2array(a(:,10)); % RL 2025
d = table2array(a(:,5)); % RL 2025
...
for  k = 1:3:163       %generates an overall picture summary; for files without alphabet as inducer, change to k=1:3:87                     
    ...
    t3 = [b(2+k),c(2+k)];
       ...
       hold on; 
       fill(x,y,1+k);
       % text(min(x),max(y),txt(1+k,5)); % RL 2025 commented
...
for  k = 1:3:30 %generates picture summary for days
    ...
    t3 = [b(2+k),c(2+k)];
       ...
       hold on; 
       fill(x,y,1+k);
       % text(min(x),max(y),txt(1+k)); % RL 2025 replaced txt(1+k,5)
