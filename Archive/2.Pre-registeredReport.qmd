...
Like in CIpora et al., 2019. Just a look at slopes.

```{r}
ID_slopeInter <- ds %>% 
  filter(Cond %in% "number") %>%
  filter(!stimulus %in% "0") %>%
  arrange(stimulus) %>%
  group_by(repetition,ID) %>%
  mutate(
    model = list(lm(y ~ x)),
...
GAID_slopeInter <- ID_slopeInter %>%
  group_by(ID) %>%
  mutate(I_M = mean(intercept,na.rm = TRUE), I_Max = max(intercept), I_Min = min(intercept), S_M = mean(slope, na.rm = TRUE), S_Max= max(slope),S_Min = min(slope),.groups = "drop")

GAID_slopeInter <- GAID_slopeInter %>%
  arrange(S_M) 

GAID_slopeInter$pos <- 1:length(GAID_slopeInter$ID)

GAID_slopeInter %>%
    ggplot(aes(y = pos, x = S_M, xmin = S_Min, xmax = S_Max, colour = group, fill = group)) +
    geom_errorbar(alpha = 0.01) +
    geom_point(alpha = 0.01, size = 1) +
    labs(
        title = "Caterpillar Plot for Slopes",
        x = "Slope [pixel]"
    ) 


GAID_slopeInter %>%
    ggplot(aes(y = pos, x = I_M, xmin = I_Min, xmax = I_Max, colour = group, fill = group)) +
    geom_errorbar(alpha = 0.01) +
    geom_point(alpha = 0.01, size = 1) +
    labs(
        title = "Caterpillar Plot for Intercepts",
        x = "Intercept [pixel]"
    ) 
```
