...
be complex and have variable geometric forms. Those geometric forms
might be different for each categories (i.e. number-forms weekdays and
months), with forms such as ellipses, zig-zags or curbed lines
[@galton1880,@flournoy1893]. SSS are usually identified using
questionnaires and consistency tests. An individual is identified as SSS
based on the variability at the stimulus level between repeated
responses: less variable responses being an indicator of the consistent
response characteristic of SSS. However, stimulus level consistency can
also be achieved by responding to the same position for all stimuli.
...
for synaesthesia in general and specifically SSS [@seron1992;
@deroy2013]. *Automaticity*: the inducer automatically triggers the
concurrent. *Unidirectionality*: while the *inducer* triggers the
concurrent, the concurrent does not trigger the inducer.
...
quantified more objectively using behavioural tasks such as consistency
...
(i.e. *inducer*-*concurrent* associations). High consistency is often
taken as evidence for synaesthesia. Consistency tests have proven
effective for colour-grapheme synaesthesia. Measures of individual
consistency can be derived using colour pickers while repeatedly
presenting the same inducer (i.e. “A”). The quantification of the
distance between concurrents from the same inducers is used to
discriminate consistent synaesthetes from controls. Specifically, the
euclidean distance in CIE L\*u\*v colour space, which is designed for
perceptual uniformity, yields optimal classification accuracy when using
a cut-off estimated as deduced from a larger representative
sample[@rothen2013a]. A similar rationale to that used for
colour-grapheme synaesthesia has been applied to characterize SSS from
consistency tasks.
...
metric for consistency. Less consistent responses leading to smaller
...
month and numbers), such as segments and polygons. These novel features
are designed to take advantage of the sequentiality or ordinality
between the responses and the geometrical properties at the spatial-form
level, see @fig-myplot00. All the consistency tests are then compared
using ROC analyses on their correct classification performances of
self-reported SSS and control groups.

![Deisgn for novel method for consistency test: from between trial
varibility to Segment and polygons formed by the space-forms using
geography tools.](RPlot02.png){#fig-Ex01 pos="'H'" fig-align="center"}
...
[https://osf.io/nu5v4](https://osf.io/nu5v4/overview)) and an additional
dataset was gently provided by private communications with Professor
...
@ward2022a; @vanpetersen2020].

#### Area and perimeter between repetitions

Formally, the area (see [@eq-area]) and perimeter (see [@eq-perim]) are
...
For the novel consistency tests, we extracted features at the
spatial-form level for each repetition separately. Taking the stimuli as
an ordered sequence we can consider them as a geometrical segments (i.e.
open geometrical form) and polygons (i.e. closed geometrical form),
similarly as originally described in @galton1880.
...
the OGC Simple Features Specification [@herring2010], a polygon boundary
is considered simple if it does not pass through the same point more
than once [@postgis]. For polygons, simplicity requires that each ring
does not self-intersect or self-touch. Note that simplicity is a
...
repetitions, hence 9 forms. The binary outcome (1 = is valid, 0 =
invalid) was then averaged across all 9 forms. Hence, it can span from 1
(all 9 are valid/simple) to 0 (none of the 9 forms are valid/simple).
Thus, the topological validity and simplicity score ranges from 0 (none
of the nine polygons are valid/simple) to 1 (all nine polygons are
valid). For example, if a participant’s polygon for weekdays were valid
across all three repetitions, months were valid in two of three
repetitions, and numbers were invalid in all repetitions, the
...
non-chronological order The form-based features computed before were
...
that genuine synaesthetes would have consistent topological features of
...
```{r, AveragePermutepp}
ds_perm_pp <- read.csv2("permuted_pp.csv")
...
```{r}
#| label: tbl-mytb03 
#| tbl-cap: "ROC analyses of all features"
#| ft.align: left
...
between the available datasets (See @fig-apx08). These differences could
be mitigated by using individually standardized coordinates M = 0.05
(*SD = 0.06*) @rothen2016, M = 0.08 (*SD = 0.15*) @ward2022a, M = 0.09
(*SD = 0.15*), Ward2, see also @tbl-mytb04.
...
*p* \< .05). Both criteria display bimodal distributions across the
groups (see @fig-apx01).
...
to higher specificity than perimeter
...
indicating poorer detection of SSS (i.e. more false negatives). See
@fig-apx01 for the densities of each standardized features suggesting
bi-modal distribution.

In sum, the permuted validity and perimeter both provide similarly good
AUC compared to the other consistency tests.

## Further analyses

We conducted further analyses by subsampling them based on the dataset
source, questionnaire percentiles. The aim of these analyses was to test
the stability of the classifications from different methods across
...
We sub-sampled the data based on the source of the data and found
different features best fit different datasets, see @fig-myplotapx02.
Interestingly the best features by datasets differ between the data
sources. While the standardized area indeed lead to the best AUC for the
data from @rothen2016, the perimeter leads to better AUC in @ward2018.
The permuted validity leads to AUC \> 80 across all datasets.

To test the stability of AUC of all features, we computed them across
several percentiles based on the questionnaire scores. Again, only the
data from Professor Ward is included there. We computed AUC, sensitivity
and sensibility on sub samples based on the percentiles of the
questionnaire scores [see @ward2018], taking the 10-0% highest
questionnaire scores against the 10 % worst (i.e. 90-100 %) . We further
sub-sampled by 10 % questionnaire scores and recalculated AUC,
sensitivity and sensibility, see @fig-myplotapx03 for AUC, DP,
sensitivity and specificity. Using this method we can see that the total
AUC remains relatively stable across percentiles.
...
see Appendix. The two highest correlations are indeed between the
questionnaire score and perimeter (r = .58) and permuted validity (r =
.50), see @fig-myplotapx04.

Some of the participants might have been originally mis-attributed to
control or SSS, see @fig-myplotapx07. Descriptively, we also found about
\~6 % of the full sample's synesthetes to be consistently classified as
controls (i.e. consistent false positive) by the five methods with the
highest AUC @fig-myplotapx08. \~3 % controls are consistently classified
as SSS (i.e. consistent false negative) @fig-myplotapx09.

We also wanted to visualize the average space form of each category at
the population level to see if there were qualitative differences
between the forms. The figure in the appendix (@fig-myplotapx06), shows
more circular patterns for months and more linear horizontal and
vertical for numbers.
...
in-principle acceptance). Hence for the stimulus levels feature we will
...
[@sachdeva2024, Stage 1 in-principle acceptance]
(<https://osf.io/6wn4m>).
...
on the dataset (see @fig-myplotapx02) it is likely that the best
features here will not match with the best feature in the to be
collected dataset.
...
perimeter on the stimulus level and permuted topological validity on the
space-form level. Hence, the area test from @rothen2016 led to worse
classification than the perimeter test with a larger sample. Furthermore
we could prove similar performances using a different approach based on
the space-form level (i.e. topological validity).

Further analyses indicates variable optimal tests in terms of AUC
depending on the datasets (see @fig-myplotapx02). Considering only the
data by Professor Ward that contains questionnaire data, we comparable
patterns of AUC and DP for more extreme Synaesthete and Control groups
when sub-sampled by questionnaire's score distribution percentiles (see
@fig-myplotapx03). Nevertheless the standardized perimeter seems to lead
to better AUC and DP in more extreme scores. Also, the standardized
perimeter lead to slightly higher correlation with the questionnaire
scores (r = .57) than permuted topological validity (r = .51), see
@fig-myplotapx04.
...
topological rules analogous to geographical and geometry-based space
...
and standardized perimeter between repetitions. Despite not yelding to
better clasisfication than stimulus level methods, this new method is
...
vs. 20%, etc.). In addition, we correlated the questionnaire scores with
the results from different features extracted from the consistency test,
see @apx-correlation-with-self-report.
...
# Feature distributions {#apx-featDensity}

Regarding the distributions of each features across the SSS vs.
controls, @fig-apx01 we compared the density plots of each standardized
criteria (in order to make them comparable) which visually suggests
...
to 100 % Specificity in Van Petersen dataset.
...
#| fig-cap: Density plots of all the features comparing SSS and controls by dataset
...
questionnaire score.\
Works only with Ward's aggregated data, see @fig-myplotapx04.
...
#| fig-cap: Synaesthetes who consistently *fail* all five tests
...
#| fig-cap: Controls who consistently *pass* all five tests
ds %>% 
