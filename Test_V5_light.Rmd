...
### 3.2.1. AUC 
```{r message=FALSE, warning=FALSE}
roc_Cons <- roc( SynQuest_NR ~ Consistency, ds[ds$dataSource =="Ward",], percent=TRUE,
            # arguments for ci
            ci=TRUE, boot.n=100, ci.alpha=0.9, stratified=FALSE,
            # arguments for plot
            plot=TRUE, auc.polygon=TRUE, max.auc.polygon=TRUE, grid=TRUE,
...
ds_long$Stage <- factor(ds_long$Stage, levels = c("C_Quest", "C_linecross", "C_ConsAndSD", "C_Cons", "C_SD", "C_permzs"))
...
# Id with no zs in one conition (i.e. responded the same position):
# ds %>%
#     filter(!ID %in% c("101_Elin","1188_MiBo","1198_LiKe","12_CH","1230_FeKe","2006_MeFu","2014_DeMl","806_AmSq","755_FrIn") )
```
...
```{r}
ds_Rothen %>%
    ggplot(aes(x = GA_lineInter,y = 1, group = Syn, fill = Syn)) +
    stat_density_ridges(quantile_lines = TRUE,quantiles = c(0.25,0.5,0.75),alpha = 0.5)


ds_Rothen %>%
    ggplot(aes(x = Consistency,y = 1, group = Syn, fill = Syn)) +
    stat_density_ridges(quantile_lines = TRUE,quantiles = c(0.25,0.5,0.75),alpha = 0.5)
```
...
*Our online screening questionnaire contains the question: BSome people always experience sequences in a particular spatial arrangement. Do you think this applies to you?^ They were also shown a single example of a drawing of a three-dimensional (3D) spatial arrangement of days of the week as an example (Fig. 3b in Rothen, Meier, & Ward, 2012). Those giving an affirmative answer were then asked which kinds of stimuli they visualize in this way*
This seems like Q1 in Ward. So I recategorized the synesthetes only based on that question:


```{r}
SynIDQ1 <- ds_Q$ID[ds_Q$`Q1 . Some people routinely think about sequences as arranged in a particular spatial configuration (as in the examples below), do you think this might apply to you? (1=strongly agree, 5= strongly disagree)` %in% 5]

ds$SynQ1 <- ds$ID %in% SynIDQ1


roc_Cons_NR<- roc(SynQ1 ~ Consistency, ds,percent=TRUE,
                # arguments for ci
                ci=TRUE, boot.n=100, ci.alpha=0.9, stratified=FALSE,
                # arguments for plot
                plot=TRUE, auc.polygon=TRUE, max.auc.polygon=TRUE, grid=TRUE,
...
roc_Cons_NR<- roc(SynQ1 ~ GA_lineInter, ds,percent=TRUE,
                  # arguments for ci
                  ci=TRUE, boot.n=100, ci.alpha=0.9, stratified=FALSE,
                  # arguments for plot
                  plot=TRUE, auc.polygon=TRUE, max.auc.polygon=TRUE, grid=TRUE,
